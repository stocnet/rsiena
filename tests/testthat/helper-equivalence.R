...
#   $control     — arguments that move into set_postest_control_saom()
...
as_object_args <- function(args) {
  ...
  out <- sp$rest
  out$uncertainty <- do.call(set_postest_uncertainty_saom, unc_call)
  if (length(sp$control) > 0L)
    out$control <- do.call(set_postest_control_saom, sp$control)
  out
}
