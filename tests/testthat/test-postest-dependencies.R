...
test_that("unsupported dependency forms are rejected, not approximated", {
  ...
  ## be silently treated as multiplicative.
  expect_match(bad(transRecTrip ~ transTrip == recip), "[Oo]nly multiplicative")
  expect_match(bad(transRecTrip ~ gw(transTrip)),      "[Oo]nly multiplicative")
  expect_match(bad(transRecTrip ~ transTrip + recip),  "[Oo]nly multiplicative")
