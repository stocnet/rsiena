 ...
 *
 * File: AverageAlterInDist2NCEffect.cpp
 *
...
#include <cmath>
#include "AverageAlterInDist2NCEffect.h"
 ...
 * Constructor.
 */
AverageAlterInDist2NCEffect::AverageAlterInDist2NCEffect(
 ...
 */
double AverageAlterInDist2NCEffect::calculateChangeContribution(int actor,
	int difference)
{
	...
	if (pNetwork->outDegree(actor) > 0)
	{
		...
		for (IncidentTieIterator iter = pNetwork->outTies(actor);
			...
			iter.next())
		{
			double alterValue = this->totalInAlterValue(iter.actor()) + pNetwork->inDegree(iter.actor()) * this->overallCenterMean();
			int tieValue =  this->pNetwork()->tieValue(actor, iter.actor());
			if (tieValue == 1)
			{
				alterValue -= this->value(actor);
			}
 ...
 */
double AverageAlterInDist2NCEffect::egoStatistic(int i, double * currentValues)
{
	double statistic = 0;
	const Network * pNetwork = this->pNetwork();
	int neighborCount = 0;
		 ...
		 iter.next())
	{
			...
			iteri.next())
		{
			if (i != iteri.actor())
			{
				alterValue += currentValues[iteri.actor()] + this->overallCenterMean();
			}
		}
// tieFromi =  this->pNetwork()->tieValue(i, iter.actor());
	...
	if (neighborCount > 0)
	{
		statistic *= currentValues[i] + this->overallCenterMean();
		if (this->ldivide1)
		{
