...
This version was renewed for \RS version 1.3.23.
\end{abstract}
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.3.23).
...
\texttt{(sameXOutAct)} defined by the
 ...
 $s^{\rm net}_{i\vit}(x) =  \sum_j x_{ij} \sum_h x_{ih}\, I\{v_i = v_h \} $;\\
 for internal effect parameter $p=2$, it is defined by\\
 $s^{\rm net}_{i\vit}(x) =  \sum_j x_{ij} \sqrt{\sum_h x_{ih}\, I\{v_i = v_h \}} $;\\
...
\verb|R CMD check RSiena_1.3.23.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.3.23.tar.gz| \\
