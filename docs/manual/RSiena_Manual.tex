...
This version was renewed for \RS version 1.3.19.
\end{abstract}
...
in function \textsf{sienaBayes()}, the reference is \citet{KoskinenSnijders2022}.
\smallskip
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.3.19).
...
numbers, as long as they are in the correct order:\textsf{RSiena}will relabel
...
or the simulated data from \textsf{siena07()} (for \texttt{i != NULL}).
...
results1      <- \textsf{siena07()}(algorithm1, data = mydata, effects = myeff)
\end{verbatim}
...
siena07ToConvergence <- function(alg, dat, eff, ans0 = NULL, ...){
   numr <- 0
   ans <-\textsf{siena07()}(alg, data = dat, effects = eff, prevAns = ans0, ...) # the first run
   repeat {
       ...
       ans <-\textsf{siena07()}(alg, data = dat, effects = eff, prevAns = ans, ...)
       }
   if (tm > 0.25)
   {
...
myans1 <-\textsf{siena07()}(estimationSettings, data = mydata, effects = myeff)
summary(myans1)
\end{verbatim}
...
ans2        <-\textsf{siena07()}(myalgorithm, data = mydata, effects = myeff, batch = TRUE)
\end{verbatim}
...
ans3  <-\textsf{siena07()}(myalgorithm, data = mydata, effects = myeff, batch = TRUE)
\end{verbatim}
...
sim_ans    <- \textsf{siena07()}(sim_model, data = mydata, effects = myeff )
\end{verbatim}
...
\begin{verbatim}
sim_ans <-\textsf{siena07()}( myalgorithm, data = mydata, effects = myeff,
                    returnDeps = TRUE )
\end{verbatim}
...
\begin{startenum}
 ...
 where $p \geq 0$ is an internal effect parameter;
...
\verb|R CMD check RSiena_1.3.19.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.3.19.tar.gz| \\
...
ans         <-\textsf{siena07()}(myalgorithm, data = mydata, effects = myeff)
summary(ans)
\end{verbatim}
}
...
5 &\textsf{siena07()} &\textsf{siena07()}(x, batch = FALSE, \newline
...
\item 2019-10-16 R-Forge Revision 341, RSienaTest version 1.2-18.
...
Change in\textsf{RSiena}and RSienaTest:
\begin{itemize}
...
Changes in\textsf{RSiena}and RSienaTest (George Vega Yon):
\begin{itemize}
...
Changes in\textsf{RSiena}as well as RSienaTest:
  \begin{itemize}
...
For\textsf{RSiena}and RSienaTest:
  \begin{itemize}
...
\end{itemize}
\item 2012-12-23 R-forge revision 221,\textsf{RSiena}and RSienaTest:
...
\item Changed version of\textsf{RSiena}to be 1.0.12 and copied all new features which
  were only in RSienaTest to RSiena.\textsf{RSiena}and RSienaTest are
...
\item 2010-06-18 R-forge revision 106: new version numbers:
  1.0.11.105 and 1.0.12.105 for\textsf{RSiena}and RSienaTest respectively.
...
Updated change log and manual in\textsf{RSiena}and \sfn{ChangeLog} in RSienaTest.
