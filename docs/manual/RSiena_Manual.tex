...
This version was renewed for \RS version 1.3.4.
\end{abstract}
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.3.4).
...
\item
  mixed twopath average effect ($X$: twopath$^{1/p}$ $W$ average)  \texttt{(outOutAvIntn)},
   defined by the outdegree of $i$
   multiplied by the average of the $X$-outdegrees of those to whom $i$ has a $W$-tie:\\
   for $p=1$, this is\\[0.4em]
...
\verb|R CMD check RSiena_1.3.4.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.3.4.tar.gz| \\
...
\item 2021-mm-dd GitHub, package version 1.x.x.
