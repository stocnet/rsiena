...
In the following, the name of the Downloads folder has to be changed
to wherever you downloaded the file and
the version number have to be amended:

For Windows:

after downloading
\url{https://github.com/snlab-nl/rsiena/releases/download/v1.2.29/RSiena_winOS.zip},
change filename \texttt{RSiena\_winOS.zip} to \texttt{RSiena.zip}, and
apply \\
\texttt{install.packages("\url{~//Downloads//RSiena.zip}", repos = NULL)}\\
(where the name \texttt{\url{~//Downloads//}} will have to be changed, as mentioned).

For Mac:
\\
\texttt{install.packages("\url{~//Downloads//RSiena\_macOS.tgz}", repos = NULL)}\\

For Unix:
\\
\texttt{install.packages("\url{~//Downloads//RSiena\_linuxOS.tar.gz}", repos = NULL)}\\
...
\verb|R CMD check RSiena_1.2-29.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.2-29.tar.gz| \\
