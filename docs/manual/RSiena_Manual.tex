...
When \sfn{siena07} finds near-perfect collinearity at the end of the
...
\label{S_sienaBayes}
  ...
  to prevent the issues that arise with such variables in normal use of \sfn{siena07}.\\
...
\sfn{sienaBayes}; see Section~\ref{S_multipleProcesses} where it is explained
that parallelization is different for \sfn{sienaBayes} than for MoM
estimation by \sfn{siena07}.
...
\item Corrections of documentation:
   \begin{itemize}
  \item Help page for \sfn{siena07} corrected with respect to \texttt{x\$lessMem}.
    \end{itemize}
\end{itemize}
...
\item Improvements of functionality:
  ...
  \item Accordingly, parameters changed that are used in \sfn{sienaAlgorithmCreate}
  ...
  (auto-correlations of statistics during Phase 3)
    added to  \sfn{sienaFit} object produced by  \sfn{siena07} for ML estimation.
    \end{itemize}
\item Corrections of documentation:
   \begin{itemize}
...
\item Bug corrections:
   ...
   \item Some bug corrections referring to the use of
     \sfn{sienaBayes}, which will be integrated later with \rs.
    \end{itemize}
\end{itemize}
...
\item Improvements of functionality:
   ...
   \item Trial values of \texttt{theta} used during Phase 2 of \sfn{siena07} added
...
Changes in RSienaTest:
\begin{itemize}
\item In \sfn{sienaBayes}: more precise check that
 ...
 \item \sfn{sienaBayes}: \texttt{zm} and \texttt{zsmall} are removed at the end.
...
\begin{itemize}
   ...
   \item In case of simulation as indicated by \texttt{x\$simOnly~=~TRUE},
     the object produced by \sfn{siena07} now also contains the standard errors of the mean,
     called \texttt{estMeans.sem} (see the help page for \sfn{siena07}).
