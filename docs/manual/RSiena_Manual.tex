...
This version was renewed for \RS version 1.6.2.
...
Robert Hellpap, James Hollway, Steffen Triebel and Daniel Gotthardt.
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.6.2).
...
This manual and the example scripts on the web
...
If and when errors in this script occur, it is good to understand
how the script operates.
It passes three times through the script:
\begin{enumerate}
...
Internal effect parametes are treated in Section~\ref{S_effpar}.
...
For a single network, the default value NULL is used
...
and the function \textsf{siena07} %and \sfn{sienacpp} both
returns a
...
is implemented in the function \sfn{siena07}.% and \sfn{sienacpp}.
...
and the dyadic covariate $U$ as \texttt{interaction2}.
...
\verb|R CMD check RSiena_1.6.2.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.6.2.tar.gz| \\
