...
\item \emph{total covariate - in - alter at distance 2} \texttt{(gwInAltDist2_nc)}.
 ...
 \item two variations of the
   ...
   and his distance 2 alters defined by
      ...
      s^{\rm beh}_{i\vit}(x, z, \alpha) \,=\,
            ...
            \, \ ;
      \end{equation}
in contrast to the next effect, this one converges towards a pure
social circle effect for $alpha \to \infty$, where the number of two-paths
connecting $i$ to his distance 2 alters is ignored.
for using no centering, work with the effect
\texttt{(totGwdspFFAlt\_nc)} respectively
\texttt{(totGwdspFBAlt\_nc)} instead;

\item {\em total incoming alter-wise geometrically weighted at distance 2 effect} \texttt{(totGwInAltDist2\_nc)},
which is a gometrically weighted version of \texttt{totInAltDist2} that penalizes 
additional distance 2 incoming alters' behavior for the same direct alter:
      \begin{equation}
      s^{\rm beh}_{i\vit}(x, z, \alpha) \,=\,
            z_{i} \,
                  \sum_{j;}^n \, x_{ij}
            ...
            \, \;
      \end{equation}
there is no centered version of this effect, as the weighting scheme assumes
integer values for the behavior kernel. With strong weighting, this effect
converges to a pure similarity effect, as only the first alter in distance 2
matters, while the number of two-paths connecting him to ego are counted
fully.

\item {\em total alter-wise geometrically weighted at distance 2 effect} \texttt{(totGwAltDist2\_nc)},
which is a gometrically weighted version of \texttt{totAltDist2} that penalizes 
additional distance 2 alters' behavior for the same direct alter:
      \begin{equation}
      s^{\rm beh}_{i\vit}(x, z, \alpha) \,=\,
            z_{i} \,
                  \sum_{j;}^n \, x_{ij}
            ...
            \, \;
      \end{equation}
this is just the FF version of \texttt{totGwInAltDist2\_nc}, which can be
used in symmetric networks to represent a geometrically weighted sructural
equivalence influence effect.
