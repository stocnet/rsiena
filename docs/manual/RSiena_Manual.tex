...
This version was renewed for \RS version 1.6.1.
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.6.1).
...
The following table gives the correspondence between the old and new names.
The `how' column indicates by the letter `A' that the change is
just an alias, or can be treated as such; the letter `C'
...
\hline
 ...
 sienaRIDynamics $\bigotimes$& interpret\_size\_dynamics  &  C   \\
\hline                  &
\emph{and for \textsf{multiSiena} } : \\
\hline
 sienaBayes             &  multi\_siena             &  ? \\
 simulateData           &  simulate                 &  ?  \\
\hline

\end{tabular}

 $\bigotimes$  = old name not retained.
\end{center}
...
\verb|R CMD check RSiena_1.6.1.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.6.1.tar.gz| \\
