...
This version was renewed for \RS version 1.4.9.
\end{abstract}
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.4.9).
...
the following effects are available.
 Currently, these effects are implemented only for bipartite (two-mode) networks.
...
\setcounter{enumi}{\value{savenumi}}
 ...
 on covariate $V$ is the same as the value of $j$ on covariate $U$,\\[0.3em]
 $s^{\rm net}_{i\vit}(x) = \sum_j x_{ij}\, I\{v_i = u_j \} $,\\[0.3em]
 where the indicator function $I\{v_i = u_j \} $ is 1 if the condition $\{v_i = u_j \} $
 is satisfied, and 0 if it is not.\\
 In practice, this effect is meaningful only if $U$ and $V$ are non-centered variables.\\
...
incoming ties sent by actors $h$ who have the same covariate value $v_h = v_i$ as $i$
 and received by those $k$ who have the same covariate value $u_k = u_j$ as $j$:\\[0.3em]
 $s^{\rm el}_{i\vit}(x) =  x_{ij}\, \sum_{h,k} x_{hk}\, I\{v_h = v_i,\, u_k=u_j \}  \ , \hfill (p=1) $\\[0.3em]
 where the indicator function $ I\{v_h = v_i,\, u_k=u_j \} $ is 1 
 if the condition $\{v_h = v_i,\, u_k=u_j  \} $ is satisfied, and 0 if it is not.
 ...
 $\displaystyle s^{\rm el}_{i\vit}(x) =  x_{ij}\, \frac{ \sum_{h,k} x_{hk}\, I\{v_h = v_i,\, u_k=u_j \}}
            { \sum_{h,k} I\{v_h = v_i,\, u_k=u_j \}} \ . \hfill (p=3) $

 For $p=2$ and $p=4$, the square root is taken: \\[0.5em]
 $s^{\rm el}_{i\vit}(x) =  x_{ij}\, \sqrt{\sum_{h,k} x_{hk}\, I\{v_h = v_i,\, u_k=u_j \}} \ , \hfill (p=2)$\\[0.3em]
 and \\[0.3em]
 $\displaystyle s^{\rm el}_{i\vit}(x) =  x_{ij}\, \sqrt{ \frac{ \sum_{h,k} x_{hk}\, I\{v_h = v_i,\, u_k=u_j \}}
            { \sum_{h,k} I\{v_h = v_i,\, u_k=u_j \}}}  \ . \hfill (p=4)$\\[0.3em]
...
\item
\begin{minipage}[t]{.70\textwidth}
   shared $W$ outdegree activity effect ($X$: shared $W$ outdegree$^{1/p}$ activity)   \texttt{(outOutDist2ActIntn)}
   defined by the outdegree of $i$
   ...
   for $p=1$, this is\\[0.4em]
$
s^{\rm net}_{i\vit}(x) =  \,  x_{i+} \sum_h (x_{h+} - \bar x)  \big\{\sum_{k} w_{ik} \, w_{hk}
                             \big\}
$\\[0.4em]
 and for $p=2$, there is a square root transformation:\\[0.4em]
 $ s^{\rm net}_{i\vit}(x) = \,  x_{i+} \sum_h \big(\sqrt{x_{h+}} - \sqrt{\bar x}\big)
            \sum_h \big\{ \sum_{k} w_{ik} \, w_{hk} \big\}$\\[0.4em]
 where $\bar x$ is the average outdegree over all waves.
\setcounter{savenumi}{\value{enumi}}
      \end{minipage}
\hfill
...
\item
 shared $W$ average outdegree activity effect ($X$: shared $W$ average outdegree$^{1/p}$ activity)
   \texttt{(outOutDist2AvIntn)},\\
   defined by the outdegree of $i$
...
\begin{minipage}[t]{.60\textwidth}
   for $p=1$, this is\\[0.4em]
$ \displaystyle
s^{\rm net}_{i\vit}(x) =  \, \frac{ x_{i+} \sum_h (x_{h+} - \bar x)  \big\{\sum_{k} w_{ik} \, w_{hk}
                             \big\} }{ \sum_{k,h} w_{ik} \, w_{hk} }
$\\[0.4em]
 and for $p=2$, there is a square root transformation:\\[0.4em]
 $  \displaystyle s^{\rm net}_{i\vit}(x) = \,  \frac{ x_{i+} \sum_h \big(\sqrt{x_{h+}} - \sqrt{\bar x}\big)
            \sum_h \big\{ \sum_{k} w_{ik} \, w_{hk} \big\} }{ \sum_{k,h} w_{ik} \, w_{hk} }
            $\\[0.4em]
 where $\bar x$ is the average outdegree over all waves.
      \end{minipage}
\hfill
...
\end{minipage}
 ...
 also to include the \texttt{inPop} and \texttt{inPopOutW} effects, 
...
\verb|R CMD check RSiena_1.4.9.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.4.9.tar.gz| \\
...
\item 2024-03-21 GitHub, package version 1.4.9
...
\item New effects:
  ...
  \item New effects \texttt{sameXV} and \texttt{sameXVInPop} for bipartite networks.
