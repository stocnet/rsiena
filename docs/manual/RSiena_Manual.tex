...
% ADD gwdspFBX gwdspFBX_nc gwdspFFX gwdspFFX_nc
...
(cf.\ (\ref{alt_av3})), \\
 ...
 \item two variations of the
   \emph{GWDSP (geometrically weighted dyadwise shared partners)} alter effects:
    {\em total GWDSP I -> K -> J alter} \texttt{(totGwdspFFAlt)} and
    {\em total GWDSP I -> K <- J alter} \texttt{(totGwdspFBAlt)}.
...
\item Two variations of the
   \emph{GWDIST2 (geometrically weighted distance 2)} alter effects:
    {\em total GWDIST2 I -> K -> J alter} \texttt{(totGwdist2FFAlt\_nc)} and
    {\em total GWDIST2 I -> K <- J alter} \texttt{(totGwdist2FBAlt\_nc)}.
...
\[
      a_{i13}(x, z) = \sum_{j}
      \begin{cases}
      1  & \text{if } x_{ij} \sum_{h} z_h  x_{hj} > 0 \\
      0 & \text{otherwise}
      \end{cases}
\]
