...
This version was renewed for \RS version 1.3.6.
\end{abstract}
...
Robert Hellpap, and Steffen Triebel.
...
an additional distribution \sfn{RSienaTest} at R-Forge, which includes
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.3.6).
...
for $p=2$ \  $s^{\rm net}_{i\vit}(x) =  \sum_j x_{ij}\, (\sqrt{w_{i+}} - \sqrt{\bar w}) =
 ...
 \texttt{(avDegIntn)}\\
 defined by the average $W$-out-degree of $i$, centered by the internal effect parameter~$p$\\[0.2em]
...
\verb|R CMD check RSiena_1.3.6.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.3.6.tar.gz| \\
