...
This version was renewed for \RS version 1.2-29.
\end{abstract}
...
\texttt{install.packages("\url{~/Downloads/rsiena\_AllOS/winOS/RSiena\_1.2.29.zip}", \\
repos = NULL)}

For Mac:

\texttt{install.packages("\url{~/Downloads/rsiena\_AllOS/macOS/RSiena\_1.2.29.tgz}", \\
repos = NULL)}

For Unix:

\texttt{install.packages(\\
"\url{~/Downloads/rsiena\_AllOS/linuxOS/RSiena\_1.2-29\_R\_x86\_64-pc-linux-gnu.tar.gz}",
 repos = NULL)}
            ...
            the root name of the file is \texttt{RSienaTest\_1.2-29}, and filling in the correct path name.
            ...
            for Windows:\\
             \texttt{install.packages("pathname to RSienaTest\_1.2-29.zip", repos = NULL, type="binary")}\\
            for Mac:\\
             \texttt{install.packages("pathname to RSienaTest\_1.2-29.tgz", repos = NULL, type="binary")}
             \item In \R from source:\\
            \texttt{install.packages("RSienaTest\_1.2-29.tar.gz", repos = NULL, type="source")}
            \item In \text{command.com} or in batch mode (Windows) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.2-29.zip}
            \item In the terminal (Mac) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.2-29.tgz}
            \item In \text{command.com} or in batch mode (Windows) or in the terminal (Mac) from source:\\
            \texttt{R CMD INSTALL RSienaTest\_1.2-29.tar.gz}
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.2-29).
...
The application of the GMoM is still in development.
...
is slightly more efficient than the MoM estimator. For the co-evolution model, 
the gain in efficiency of the GMoM relative to the MoM is usually larger. 
...
The comparison of the GMoM with ML and Bayesian 
...
with the function \sfn{siena07()} is obtained by specifying the GMoM statistics 
and the option \texttt{gmm=TRUE} of the function \sfn{sienaAlgorithmCreate()}. 

Compared to the regular MoM, the GMoM allows using more statistics than parameters, 
...
complement the information provided by the regular statistics by considering two consecutive 
...
on the network and behavior by considering the network and behavior at the same time point.
...
one should keep in mind that the GMoM statistics are not model effects. 
They are only used to evaluate the moment conditions and their use does not extend 
the specification of the probability model. 
...
The estimation of the GMoM weights requires a larger number of simulations 
in phase 1 and phase 3 and (at least) one iteration of the function \sfn{siena07}.
...
The code below illustrates the estimation procedure described in the previous lines.

\begin{verbatim}
# Step 1
myalgorithm <- sienaAlgorithmCreate(nsub=2, n3=5000, gmm=TRUE)
ans <- siena07(myalgorithm, data=mydata, effects=myeff, initC=TRUE)
ans

Step 2
myalgorithm <- sienaAlgorithmCreate(nsub=4, n3=5000, gmm=TRUE)
