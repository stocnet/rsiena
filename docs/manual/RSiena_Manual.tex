...
This version was renewed for \RS version 1.6.5.
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.6.5).
...
The output file \texttt{mydata\_out.txt},
mentioned in Section~\ref{S_output},
also contains so-called \emph{one-step estimates}.
...
\end{enumerate}
  The function \textsf{multi\_siena} is time-consuming. When starting to use it, it is advisable
...
\verb|R CMD check RSiena_1.6.5.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.6.5.tar.gz| \\
...
In the function \textsf{make\_specification} in file \sfn{effects.r}, some additional
