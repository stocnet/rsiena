...
This version was renewed for \RS version 1.2.30.
\end{abstract}
...
The source code for \RS is at GitHub:\\
...
Felix Sch\"{o}nenberger, Viviana Amati,
Christian Steglich, Johan Koskinen, Mark Ortmann, Nynke Niezink,
James Hollway, and Robert Hellpap.
...
The development of \RS was carried out by researchers 
working at the Universities
of Oxford, Groningen, Konstanz,  Z\"{u}rich, and Pittsburgh (CMU).
...
\url{http://cran.r-project.org/web/packages/RSiena/} .\\
            ...
            \texttt{.tgz} for Mac, \texttt{.tar.gz} for Unix/Linux),
            then using the binary is recommended.
            Installation from binary is much faster than installation from source.

            Installation from binary can be done as follows.
        Download  the latest release for your computer system from
        \url{https://github.com/snlab-nl/rsiena/releases/}
        or  \url{http://www.stats.ox.ac.uk/~snijders/siena/siena_downloads.htm}. \\

        For Windows:

        the downloaded file is \texttt{RSiena\_1.2.30.zip} for 
        downloads from the Siena website 
        and \texttt{RSiena\_winOS.zip} for downloads from GitHub. In the latter case,
        please rename the file to \texttt{RSiena\_1.2.30.zip}.
        Then you can install it by the \R command 
\begin{verbatim}
install.packages(pathname to "RSiena_1.2.30.zip", repos = NULL))
\end{verbatim}

        For Mac:

        the downloaded file is \texttt{RSiena\_1.2.30.tgz}. 

        It can be installed by the \R command 
\begin{verbatim}
install.packages("pathname to RSiena_1.2.30.tgz", repos = NULL))
\end{verbatim}

        For Unix:

        the downloaded file is \texttt{RSiena\_linuxOS.tar.gz}.

        It can be installed by the \R command 
\begin{verbatim}
install.packages("pathname to RSiena_linuxOS.tar.gz", repos = NULL))
\end{verbatim}
\medskip
            ...
            the root name of the file is \texttt{RSienaTest\_1.2.30}, and filling in the correct path name.
            ...
            for Windows:\\
             \texttt{install.packages("pathname to RSienaTest\_1.2.30.zip", repos = NULL, type="binary")}\\
            for Mac:\\
             \texttt{install.packages("pathname to RSienaTest\_1.2.30.tgz", repos = NULL, type="binary")}
             \item In \R from source:\\
            \texttt{install.packages("RSienaTest\_1.2.30.tar.gz", repos = NULL, type="source")}
            \item In \text{command.com} or in batch mode (Windows) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.2.30.zip}
            \item In the terminal (Mac) from binary:\\
            \texttt{R CMD INSTALL RSienaTest\_1.2.30.tgz}
            \item In \text{command.com} or in batch mode (Windows) or in the terminal (Mac) from source:\\
            \texttt{R CMD INSTALL RSienaTest\_1.2.30.tar.gz}
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.2.30).
...
\end{itemize}
For both methods, the standard errors are calculated on the basis of
Phase~3 of the algorithm, and the length of this phase, given by the
value of \texttt{n3} in function \sfn{sienaAlgorithmCreate}, determines
the precision of the estimation of the standard errors.
...
Whether the given value of \texttt{n3} is adequate to estimate the
standard errors with sufficient precision can be checked using the function
in script \\
\url{http://www.stats.ox.ac.uk/~snijders/siena/SE\_checks.R},\\
of which the use is demonstrated in script\\
...
which creates the object with specifications for the algorithm.
...
\item 2021-02-24  GitHub, package version 1.2.30.\\
