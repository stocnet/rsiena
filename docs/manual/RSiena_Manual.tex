...
This version was renewed for \RS version 1.3.18.
\end{abstract}
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.3.18).
...
\item \emph{in-isolate Outdegree effect}, (\texttt{inIsDegree}), the
 ...
 for internal effect parameter $p=2$, it is defined by\\
 $f^{\rm net,\, el}_{i\vit}(x)  =   \sqrt{ x^{(r)}_{i} - \bar x^{(r)}} $ \ ; \\
 since this is an elementary effect, it can be used in interactions
 and in endowment and creation functions;
...
\verb|R CMD check RSiena_1.3.18.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.3.18.tar.gz| \\
...
\item Improvements of functionality:
  ...
  \item List element \texttt{ac3} added to \sfn{sienaFit} object if \texttt{maxlike}.
    \end{itemize}
\item Corrections of documentation:
   \begin{itemize}
