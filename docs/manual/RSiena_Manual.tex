...
\usepackage{times}
\usepackage[T1]{fontenc} % for searchable underscores in pdf
...
% We may change the following; now: table of contents lists up to subsection level
\setcounter{tocdepth}{4}

% To allow \paragraph as a level below \section
\setcounter{secnumdepth}{4}

% for the following: latex.ltx says
...
\renewcommand\paragraph{\@startsection
   ...
   {3ex \@plus1ex \@minus.2ex}       % beforeskip
   {2ex \@plus1ex \@minus.2ex}       % afterskip
...
\author{\Large Ruth M.\ Ripley, Tom A.B.\ Snijders\\[1ex]
        \Large Zs\'{o}fia Boda, Andr\'{a}s V\"{o}r\"{o}s  \\[4ex]
...
This version was renewed for \RS version1.6.9.

In version 1.6, the names of \RS functions have undergone
important changes. This manual is for the new names.
\end{abstract}
...
in function \sfn{multi\_siena}, the reference is \citet{KoskinenSnijders2023}.
\smallskip
...
\label{S_estim}
\noindent
\RS estimates parameters by the function \sfn{siena},
%and (alternatively) \sfn{sienacpp}
...
\item In Phase~3, the final result of Phase~2 is used, and it is checked if the
  ...
  argument \texttt{prevAns} in the call of \sfn{siena}.
 % (or \sfn{sienacpp}).
  See the help page for  \sfn{siena}.
\end{enumerate}
\end{enumerate}
...
\subsection{Using \RS within \Rn}
   ...
   \begin{enumerate}
	...
	\begin{enumerate}
		\item Network data should be in objects of class matrix.
            An alternative is to use sparse matrices of class
             \texttt{"TsparseMatrix-class"}  from package { \sf Matrix}.
        ...
        again, an alternative is sparse matrices of class
             \texttt{"TsparseMatrix-class"}.
	\end{enumerate}
   ...
   be two node sets.
	...
	\begin{enumerate}
		\item	\sfn{as\_dependent\_rsiena} for networks and behavior variables;
		\item	only for two-mode networks, \sfn{as\_nodeset\_rsiena} for defining nodesets;
		\item	\sfn{as\_covariate\_rsiena} and \sfn{as\_covariate\_rsiena} for constant and
                changing/varying individual covariates respectively;
		\item	\sfn{as\_covariate\_rsiena} and \sfn{as\_covariate\_rsiena} for
	...
	\item	Create a \textsf{siena} data object containing all the objects
            specified above using the function \sfn{make\_data\_rsiena}
            (see Section~\ref{S_datatypes}).
	\item	Use \sfn{make\_specification} to create an effects object.
	...
	\item	You can use various algorithm settings.
            Alternative settings of the estimation algorithm can be defined by
            \sfn{set\_algorithm\_saom} (see Section~\ref{S_param});
            special model definitions
            can be chosen using function \sfn{set\_model\_saom};
            and output options can be chosen by \sfn{set\_output\_saom}.
            For a first model try these can be skipped.
	\item	Use \sfn{write\_report} to produce an output file
                 presenting some descriptive statistics
                for the objects included in the model.
	\item	Use functions \sfn{set\_effect}, \sfn{set\_effect} and
            \sfn{set\_interaction} to further specify the model
            (see Sections~\ref{S_imp_str1}~--~\ref{S_eff_beh}).
	\item	Use \sfn{siena} %or \sfn{sienacpp}
	...
	\item	Basic output is written to a text file in the actual working directory.
             The default filename is the name of the data object
             to which \texttt{\_out.txt} is appended. Results can also be
...
\begin{itemize}
    \item \sfn{basicRSiena.r}: a minimal example of a basic sequence of commands
           for estimating a model by function \sfn{siena} of \rs.
...
\item Look at the start of the output file obtained from
       \sfn{write\_report} for general data
...
\item When parameters have been estimated, first look at the
     \texttt{overall maximum \\convergence ratio} and the \texttt{t statistics for deviations
      from targets}.
      We say that the estimation has converged if the former is less than 0.25,
...
\item In rare circumstances, when the data set leads to instability
      ...
      in function \textsf{set\_algorithm\_saom}.
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g., 1.6.9).
...
\section{New function names from Version 1.6}
\label{S_names}

In Version 1.6, many functions in \RS have been given new names.
This was done in order to make them more readily comprehensible,
to give a somewhat more logical structure, and
for coordination with other packages in the \SN suite.
The old function names still are available in order
for existing scripts still to be useful also for the newer versions of \rs.

The same holds for \MS since version 1.2.36.

Some example scripts on the web
still use mainly the old function names,
but this soon will be changed.
The descriptions of the old and the new function names
are in the same help pages in the R system.
\medskip

For many functions this is just a name change.
The main differences are the following.
\begin{enumerate}
  \item The algorithm now does not have to be specified for
        standard use. The name of the output text file generated
        by the estimation is called \texttt{dataName\_out.txt},
        where \texttt{dataName} is the name of the data object.
  \item Constructing the algorithm is split into three functions:
        \begin{enumerate}
        \item \sfn{set\_model\_saom} for defining the general model
             (e.g., \texttt{modelType});
        \item \sfn{set\_algorithm\_saom} for defining the
              method and algorithm for estimation (e.g., settings of the
              Robbins-Monro algorithm, maximum likelihood estimation);
        \item \sfn{set\_output\_saom} for defining the output to be
               created (e.g., different name for the text output file).
        \end{enumerate}
  \item The estimation function now is called \sfn{siena}
        instead of \sfn{siena07}.
        Its first argument is the data set; omitting the algorithm objects
        is usual, and will lead to a default estimation.
  \item The estimation function in \MS now is called \sfn{multi\_siena}
        instead of \sfn{sienaBayes}. The help page\\
        \texttt{?multi\_siena}\\
        gives enough information.
  \item Constructing covariates is done by one function,
        \sfn{as\_covariate\_rsiena}; the distinction between monadic and dyadic,
        and between changing or time-constant, is made by the
        structure of the input object and the further arguments
        (see the table below).
  \item Including effects is done by one function, \sfn{set\_effect}.\\
        If more than one effect is to be included by one function call,
        the \texttt{shortNames} are to be given in a list.\\
        The argument names now are \texttt{depvar}, \texttt{covar1},
        and \texttt{covar2}, replacing the earlier (less fortunate)
        names \texttt{name}, \texttt{interaction1}, and \texttt{interaction2}.
  \item For defining interactions, the function \sfn{set\_interaction}
        requires to give the \texttt{shortNames} of the two or three
        interacting effects in a list.\\
        Like for \sfn{set\_effect}, argument names now are
        \texttt{depvar}, \texttt{covar1}, and \texttt{covar2}.
\end{enumerate}

In departure of the general rule that the old names are retained
in view of compatibility, the following names were canceled:
\begin{enumerate}[resume]
  \item The names \sfn{sienaRI} and \sfn{sienaRIDynamics}
        are not retained, and replaced by \sfn{interpret\_size} and
        \sfn{interpret\_size\_dynamics}; these functions still are not
        final.
  \item The name \sfn{siena.table} is not retained but replaced
         by  \sfn{siena\_table};
         however, the new (and preferred) name is \sfn{write\_result}.
  \item \sfn{model.create}, an alias for \sfn{sienaAlgorithmCreate},
        was dropped.
\end{enumerate}

\noindent
Table \ref{T_newnames} gives the correspondence between the old and new names.
\bigskip
...
\hline
 sienaDependent         &  as\_dependent\_rsiena     & A\\
 coCovar                &  as\_covariate\_rsiena     & A \\
 varCovar               &  as\_covariate\_rsiena(..., type="monadic")     &  C \\
 coDyadCovar            &  as\_covariate\_rsiena(..., type="oneMode")     &  C \\
                        &  \emph{or} as\_covariate\_rsiena(..., type="bipartite")     &  \\
 varDyadCovar           &  as\_covariate\_rsiena     &  A \\
 sienaCompositionChange &  as\_composition\_rsiena   &  A \\
 sienaCompositionChangeFromFile &  as\_composition\_file\_rsiena  & A \\
 sienaNodeSet           & as\_nodeset\_rsiena        &  A \\
 sienaDataCreate        & make\_data\_rsiena         &  A \\
 sienaGroupCreate       & make\_group\_rsiena        &  A \\
 getEffects             & make\_specification        &  A \\
 print01Report          & write\_report              &  A  \\
 includeEffects         & set\_effect                &  C  \\
 setEffect              & set\_effect                &  C \\
 includeInteraction     & set\_interaction           &  C \\
 sienaAlgorithmCreate   & set\_model\_saom,          &  \\
                        & set\_algorithm\_saom,      &  \\
                        & set\_output\_saom          &  \\
 sienaDataConstraint    & make\_constraint           &  A \\
 siena07                & siena                      &  C \\
 siena08                & meta\_siena                &  A \\
 siena.table  $\bigotimes$      & write\_result      &  A \\
 Multipar.RSiena        & test\_parameter            &  A \\
 Wald.RSiena            & test\_parameter            & C \\
 testSame               & test\_parameter(..., method="same", ...)    & C \\
 score.Test             & test\_parameter(..., method="score", ...)   & C  \\
 sienaTimeTest          & test\_time                 &  A \\
 sienaGOF               & test\_gof                  &  A \\
 descriptives.sienaGOF $\bigotimes$ & descriptives               &  A \\
 selectionTable         & interpret\_selection       &  A \\
 influenceTable         & interpret\_influence       &  A \\
 sienaRI  $\bigotimes$  & interpret\_size            &  C \\
 sienaRIDynamics $\bigotimes$& interpret\_size\_dynamics  &  A   \\
 updateTheta            & update\_theta              &  A \\
 updateSpecification    & update\_specification       &  A \\
\hline                  &
\emph{and for \textsf{multiSiena} } : \\
\hline
 sienaBayes   $\bigotimes$           &  multi\_siena             &  C \\
 simulateData  $\bigotimes$          &  simulate                 &  C  \\
 glueBayes    $\bigotimes$           &  multi\_glue              & A  \\
 extract.sienaBayes $\bigotimes$     & extract\_sienaBayesFit       & A  \\
 extract.posteriorMeans $\bigotimes$ & extract\_posteriorMeans   & A  \\
\hline
\end{tabular}
  \caption{Correspondence between old and new function names.\protect\newline
  `how' column: `A' means that the change is
just an alias, or can be treated as such;  `C'
indicates that the arguments also changed.
 $\bigotimes$ means that the function with the old name is not retained.}
 \label{T_newnames}
\end{table}

\clearpage

\subsection{Function \sfn{transformScript}}
\RS contains function \sfn{transformScript}, which
transforms R scripts with calls of \RS and \MS
functions to scripts utilizing the new names.
See its help page!

The function \sfn{transformScript} operates on the script (or other text files,
e.g., \LaTeX)  line by line.
It gives output in the new script line by line, with an exception
for calls of \sfn{sienaAlgorithmCreate}.
As this function now is split into three, no line in the new script
is created; but calls of \sfn{siena07} and \sfn{sienaBayes} are
checked for the algorithms they use, and transformed into  calls of \sfn{siena}
and \sfn{mult\_siena}, respectively, preceded by calls of
\sfn{set\_model\_saom}, \sfn{set\_algorithm\_saom}, and/or
\sfn{set\_output\_saom},
corresponding to the algorithm object used by this call of
\sfn{siena07} or \sfn{sienaBayes}.
These algorithm objects constructed always have the names, respectively,
\texttt{alg\_model}, \texttt{alg\_alg}, and \texttt{alg\_out}.
\medskip

Function \sfn{transformScript} also produces a log file.
If it leads to an error, it may be helpful to use\\
\texttt{traceback()}\\
immediately after the error occurs; and to look at
the log file, which will have line numbers until the last
line that was treated.
\bigskip

If you want to transform to the new names
all files in the working directory with extension name \texttt{.R},
...
If and when errors in a newly created script occur,
it is good to understand how the function  \sfn{transformScript}
operates.
It passes three times through the old script:
\begin{enumerate}
  \item The first time, to replace the names of functions in category
    'A' in Table~\ref{T_newnames}.
  \item In the second pass,   calls of \RS and
     \MS functions in category 'C'
     are transformed to calls utilizing the new names.\\
     The only \RS functions in the script
     that are executed themselves here are calls of \sfn{sienaAlgorithmCreate};
     their results are used for transforming
     calls of \sfn{siena07} and \sfn{sienaBayes}.
     All  function calls in category 'C' except those of
      \sfn{sienaAlgorithmCreate} are replaced with
      calls of the new functions.
  \item In the final pass, if any old function names are left and used
     without being a function call (i.e., without being
     followed by "(")), they are replaced with the new names.
\end{enumerate}
...
\begin{enumerate}
 ...
 \item estimation of parameter values using stochastic simulation
    ...
    \textsf{\textsl{dname}\_out.txt}, where \textsf{\textsl{dname}} is the name
   of the data set; a different name can be specified
   using function  \sfn{set\_output\_saom};
...
the functions \sfn{as\_dependent\_rsiena}, \sfn{as\_covariate\_rsiena}, \sfn{as\_covariate\_rsiena},
\sfn{as\_covariate\_rsiena}, \sfn{as\_covariate\_rsiena}, or \sfn{as\_composition\_rsiena}.
Second, the variables must be combined into one \sfn{siena} data set by the function
\sfn{make\_data\_rsiena}.
...
\texttt{type} in  the \textsf{as\_dependent\_rsiena} function, which for
...
\textsf{as\_dependent\_rsiena}.


For data specification by  \textsf{as\_dependent\_rsiena}, the network
must be specified as a matrix or array or list of sparse matrix of triples.
The help page for \textsf{as\_dependent\_rsiena} has an example of input
by sparse matrices.

For data specification by
%the graphical interface \textsf{siena01Gui} (documented separately) or by
the function \textsf{make\_data\_rsienaFromSession},
...
\sfn{as\_dependent\_rsiena}, have for running the estimations
the same efficiency as Pajek or Siena format.
%Data input as edge lists will also be made available by means of the
%\textsf{as\_dependent\_rsiena} function (in some future version of \sn).
The three possible formats for digraph input are as follows.
\begin{enumerate}
\item \emph{Adjacency matrices}.\\
      These can be used in  \textsf{as\_dependent\_rsiena} and in
      \sfn{make\_data\_rsienaFromSession}.\\
      ...
      The help file for \textsf{as\_dependent\_rsiena} shows by examples how the
      specification can be given by sparse matrices.
\iffalse
    \item \emph{Pajek format}.\\
      These can be used in \sfn{make\_data\_rsienaFromSession}
    ...
    \item \emph{Siena format}.\\
      These can be used in \sfn{make\_data\_rsienaFromSession} (only available in \rst).\\
...
them \texttt{1:nwaves}. And use \sfn{make\_data\_rsienaFromSession} to read the files in.
...
\textsf{make\_data\_rsienaFromSession} (only available in \rst),
...
or the simulated data from \sfn{siena} (for \texttt{i != NULL}).
...
respectively, for argument \texttt{type} in \sfn{as\_dependent\_rsiena}.
...
given by \sfn{write\_report} and the values will be truncated towards zero.
...
\sfn{as\_covariate\_rsiena} in the case they are constant over time,
and \sfn{as\_covariate\_rsiena} if they are changing over time.
...
in a separate call of \sfn{as\_covariate\_rsiena}, using for input
...
This can be turned off by constructing the covariate with the argument
...
\sfn{as\_covariate\_rsiena} and \sfn{coDyadCovar},
...
\sfn{as\_covariate\_rsiena}, and changing dyadic covariates by \sfn{as\_covariate\_rsiena}.
...
\end{equation}
      ...
      \textsf{make\_data\_rsiena} and stored as the \sfn{simMean} attribute
      of \texttt{mydata\$cCovars\$myvar}, where \texttt{mydata} is the name
      of the object created by \textsf{make\_data\_rsiena}, and \texttt{myvar}
      is the name of the variable used as the argument for
      \textsf{make\_data\_rsiena}, while the name \texttt{cCovars} applies for
...
by function \textsf{make\_data\_rsiena}. The centered values then are stored
(see below),
...
\texttt{centered=FALSE} in the call of \sfn{as\_covariate\_rsiena},
\sfn{as\_covariate\_rsiena}, \sfn{as\_covariate\_rsiena}, or \sfn{as\_covariate\_rsiena}, respectively.
...
within-group mean;
actor covariates in multi-group data sets  created by \sfn{make\_group\_rsiena}
...
It is also reported in the
 output file produced by \sfn{write\_report} and available -- e.g., for the first
...
For multi-group data sets constructed by function \sfn{make\_group\_rsiena},
...
Dependent behavior variables are not centered by \textsf{make\_data\_rsiena},
...
in any given period,
and mention this in the output file generated by \textsf{write\_report}.
...
in the call of \textsf{as\_dependent\_rsiena} (see the help file for this function).

If a dependent variable is only increasing or only decreasing
for all periods and \textsf{as\_dependent\_rsiena} was called with
...
by calling \textsf{as\_dependent\_rsiena} with \sfn{allowOnly=FALSE}.
...
the output file of \textsf{write\_report} --- which is given only,
...
by the functions \sfn{as\_covariate\_rsiena} or \sfn{as\_covariate\_rsiena}, the user
...
By using the argument \texttt{imputationValues} in \sfn{as\_covariate\_rsiena}
and \sfn{as\_covariate\_rsiena}, other values
...
are \sfn{as\_composition\_rsienaFromFile} in case a file is used,
and \sfn{as\_composition\_rsiena} in case a list is used.
How to use a separate input file,
called the \emph{exogenous events file}, is
described in the help page for \sfn{as\_composition\_rsienaFromFile}.
...
\sfn{as\_composition\_rsienaFromFile} or
\sfn{as\_composition\_rsiena} is of class
...
does not combine properly with the \textsf{test\_gof} function
(Section~\ref{S_gof}).
...
\textsf{test\_gof}  will work properly.
...
functions \sfn{set\_effect} and \sfn{set\_effect}
are only \texttt{eval}, \texttt{creation}, and \texttt{endow}.
...
The effects object is originally created by the function \textsf{make\_specification} and
subsequently modified by functions \sfn{set\_effect},
\sfn{set\_effects}, and others.
...
the name of the dependent variable (\sfn{depvar}) also is required
to specify the effect, and will be given in the column \sfn{covar1}
...
The \sfn{type} can be set in the functions \sfn{set\_effect},
\textsf{set\_effect}, and \sfn{set\_interaction}.
...
\end{minipage}
      ...
      \item[c.] The \emph{gwesp} effect; see later in this manual.
                ...
                parameter of 69 (see Section~\ref{S_effpar} for internal effect
                parameters).
...
\item The \emph{ reciprocal degree activity effect} (where internal effect
     ...
     see Section~\ref{S_effpar} for internal effect parameters).
...
output file produced by \sfn{write\_report}
...
\texttt{siena} object in \textsf{make\_data\_rsiena}.
When one of the constraints is true,
this will be reported in the output of \textsf{write\_report}.
...
(e.g., turned off) by function \sfn{make\_constraint}.
...
they do not work for \textsf{multi\_siena}.
...
\item Unilateral initiative and reciprocal confirmation,
      ...
      \textsf{set\_model\_saom}. This parameter must be given as a
      named vector; see the help page for \textsf{set\_model\_saom}.
...
This is elaborated for network effects in Section~\ref{S_intnet}
and for behavior effects in Section~\ref{S_beh_infl}.

Interactions can be specified by the function \sfn{set\_interaction}.
...
and is set in the call of \sfn{make\_specification}.
\medskip
...
Internal effect parameters are treated in Section~\ref{S_effpar}.
Function \sfn{set\_effect} has an argument \texttt{parameter} to set the
internal effect parameter, but the function  \sfn{set\_interaction} does not
have this argument. The reason is that the internal effect parameters for the
...
\texttt{set\_effect(..., parameter=..., include=FALSE)}\\[0.2em]
...
Examples of the use of \sfn{set\_interaction} are as follows.
\begin{verbatim}
...
\end{verbatim}
Note the \texttt{covar1} argument; this argument is used also
when defining these effects using \sfn{set\_effect}.
In this case, however, two effects are defined,
and accordingly the \texttt{covar1} argument has two components,
combined by the \sfn{c()} function.
For effects such as \texttt{recip} that need no \texttt{covar1}
argument, the corresponding string is just the empty string, \texttt{""}.

Interactions between three effects are defined similarly,
but now the \texttt{covar1} argument must combine three components.

The number of network interaction effects that can be created is defined by the
argument \texttt{nintn} in function \sfn{make\_specification}. If you need more
...
again using the function \textsf{set\_interaction}.
...
argument \texttt{behNintn} in function \sfn{make\_specification}. If you need more
interactions than permitted, just make a new effects object with the
...
This can be tested by the \textsf{test\_time} function, as explained
...
the score type test that is implemented in the \textsf{test\_time} function,
...
in the \textsf{MaxDegree} parameter of the \textsf{set\_model\_saom}
function, which determines the details of the model specification.
...
estimation; therefore, neither ML estimation using \textsf{siena}
nor multilevel estimation using \textsf{multi\_siena} support
this option.
...
\end{verbatim}
(On the log-probability scale, a contribution of \texttt{--30}  is
practically minus infinity.)

\newpage

\subsection{Goodness of fit with auxiliary statistics: \textsf{test\_gof}}
\label{S_gof}

The function  \sfn{test\_gof} in \RS permits users
...
The \textsf{test\_gof} function, proposed and elaborated by
...
The examples in the help pages for \textsf{test\_gof} and \sfn{sienaGOF-auxiliary}
...
on the help page for  \sfn{test\_gof-auxiliary} to see how this can be used
...
\begin{scope}
\boldmath
\tikzstyle{every node}=[x=5ex,y=5ex,inner sep=0pt,minimum size=1ex,shape=circle,draw,fill=black]
\tikzstyle{every path}=[very thick, -stealth', shorten <=1.5pt,shorten >=1.5pt, ->]
\node (i) at (0,0)  {.};
\node (j) at (2,0) {.};
\node (k) at (1,1.73) {.} ;
\node[fill=none, draw=none] (t) at (1,-0.7) {003};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
\begin{scope}
\boldmath
\tikzstyle{every node}=[inner sep=0pt,x=5ex,y=5ex,minimum size=1ex,shape=circle,draw,fill=black]
\tikzstyle{every path}=[very thick, -stealth', shorten <=1.5pt,shorten >=1.5pt, ->]
\node (i) at (0,0)  {.};
\node (j) at (2,0) {.};
\node (k) at (1,1.73) {.} ;
\draw (i) -> (k)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {012};
\end{scope}
\end{tikzpicture}
\bigskip
...
\draw (i) -> (j)  ;
\draw (j) -> (i);
\node[fill=none, draw=none] (t) at (1,-0.7) {102};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
...
\draw (k) -> (j)  ;
\draw (k) -> (i);
\node[fill=none, draw=none] (t) at (1,-0.7) {021D};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
...
\draw (i) -> (k)  ;
\draw (j) -> (k);
\node[fill=none, draw=none] (t) at (1,-0.7) {021U};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
...
\draw (i) -> (k)  ;
\draw (k) -> (j);
\node[fill=none, draw=none] (t) at (1,-0.7) {021C};
\end{scope}
\end{tikzpicture}
\bigskip
...
\draw (j) -> (i);
\draw (k) -> (j)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {111D};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
...
\draw (j) -> (i);
\draw (j) -> (k)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {111U};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
...
\draw (i) -> (j);
\draw (j) -> (k)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {030T};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
...
\draw (j) -> (k);
\draw (k) -> (i)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {030C};
\end{scope}
\end{tikzpicture}
\bigskip
...
\draw (i) -> (j);
\draw (j) -> (i)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {201};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
...
\draw (i) -> (j);
\draw (j) -> (i)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {120D};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
...
\draw (i) -> (j);
\draw (j) -> (i)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {120U};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
...
\draw (i) -> (j);
\draw (j) -> (i)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {120C};
\end{scope}
\end{tikzpicture}
\bigskip
...
\draw (j) -> (i)  ;
\draw (j) -> (k)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {210};
\end{scope}
\end{tikzpicture}
\hspace{5ex}
\begin{tikzpicture}
...
\draw (j) -> (k)  ;
\draw (k) -> (i)  ;
\node[fill=none, draw=none] (t) at (1,-0.7) {300};
\end{scope}
\end{tikzpicture}
\end{center}
...
\subsubsection{Plots}
The results of \texttt{test\_gof} can be plotted which then produces
...
The $p$-values for \textsf{test\_gof} compare, in the space of outcomes
...
\textsf{test\_gof} needs to be applied to each group separately,
using the \texttt{groupName} argument, usually
\texttt{groupName="Data1"},  \texttt{groupName="Data2"}, etc.\
(see the help page for \textsf{test\_gof}).

For combining \textsf{test\_gof} results for multiple groups,
...
%For the $p$-value of test\_gof, consult the help page of sienaGOF.
...
\subsubsection{Composition change, missing data, and structural values in \textsf{test\_gof} }


The method of joiners and leavers for representing composition change
(Section~\ref{S_comp}) does not combine properly with the \textsf{test\_gof} function.
...
in \textsf{test\_gof}. The aim is that such values do not contribute
...
The estimation process is implemented in function \textsf{siena},
using an algorithm specification defined in function \textsf{set\_algorithm\_saom}.
Estimation starts with initial values for the parameters,
and the function \textsf{siena} returns a
...
\iffalse
Much of what follows is about the use of \sfn{siena}
...
`back end' \sfn{C++} part of \si, contrasting with \sfn{siena}
...
\end{verbatim}
The function \textsf{set\_algorithm\_saom} can be used to  define
a different algorithm for the estimation; and function  \textsf{siena}
...
\ref{S_Est}.
(If you are running \RS from \texttt{RStudio}, this screen is visible
when you click on the little feather symbol that appears on the right-hand side
of the taskbar when the estimation is running.)
It is possible to intervene in the algorithm by clicking on the
appropriate buttons:
the algorithm may be restarted or terminated. In most cases
this is not necessary.
...
Experience shows that parameters estimated by \sfn{siena} are
...
the default threshold value of 50 can be changed by the argument
...
\subsection{Arguments of the algorithm}
\label{S_param}

Function \textsf{set\_algorithm\_saom} defines the algorithm.
The algorithm is described more in detail in Section~\ref{S_algorithm}.
This function has a lot of arguments (keywords) that you will never need to touch,
and which have been put there for the purpose of investigating
the best settings of the algorithm.
The following arguments are worth knowing about.
\begin{enumerate}
\iffalse
...
\fi
  ...
  \item \texttt{seed}: the seed of the random number algorithm. This seed determines
       ...
       argument \texttt{n3} is large enough, different random seeds will give
       ...
       argument \texttt{n3} is large enough.)
\end{enumerate}
Some other arguments are sometimes important in the case of convergence
...
This is determined by the following arguments.
\begin{enumerate}[resume]
...
`no change'). The associated arguments are
\begin{enumerate}[resume]
...
\end{enumerate}
Furthermore, there is an argument with special purposes that sometimes
can be used.
\begin{enumerate}[resume]
  \item \texttt{simOnly}: to indicate that the interest is in simulation,
       not estimation; see Chapter~\ref{S_sim}.
\end{enumerate}
Finally, there is an argument for which the default is always good;
...
The default name is determined by the name of the data set.
For instance, if the data set has name \texttt{mydata}, the output file
will be called \texttt{mydata\_out.txt}.
Other names can be set by function \sfn{set\_output\_saom}.
...
one runs \sfn{siena}, if necessary repeatedly with
subsequent runs using the \texttt{prevAns} argument as above,
...
The \emph{initial values} can be determined in three ways.
\begin{enumerate}
\item The default: if \texttt{useStdInits = FALSE} and no \textsf{prevAns}
       argument is given in the call of \textsf{siena},
...
\end{verbatim}
      ...
      As long as no time dummies have been requested using\\
       \sfn{sienaTimeFix}, the initial values for the requested
      effects are in the vector
\begin{verbatim}
myeff$initialValue[myeff$include]
\end{verbatim}
      Changing these values is not often necessary, because the
      argument \textsf{prevAns}, as explained in the next item,
      ...
      function \textsf{update\_theta}, which copies the
...
results1 <- siena(data=mydata, effects=myeff, prevAns=results0)
      ...
      \textsf{prevAns} argument. This must be a
...
\item If \texttt{useStdInits = TRUE} is used in the call of
      \textsf{set\_algorithm\_saom}, standard initial values are used.\\
...
argument, while taking care that  \texttt{useStdInits = FALSE}
...
Above, the \sfn{prevAns} argument was mentioned which will
...
arguments in the \sfn{set\_algorithm\_saom} function, to try and
improve convergence.
The main arguments of \sfn{set\_algorithm\_saom} that can be used
...
\item \texttt{n3}\\
    The number of steps in Phase 3 should be higher for complicated models.
    The estimation of the overall maximum convergence $t$-ratio is biased
    upward, and the bias is reduced for larger values of \texttt{n3}.
\item \texttt{diagonalize}\\
    This argument may range from 0 to 1,
...
\item \texttt{doubleAveraging}\\
      ...
      Mostly the default value is good and does not need to be changed.
\end{itemize}
...
\sfn{update\_theta} to copy the results from the earlier estimation
...
while using the \sfn{prevAns} argument to use previous estimates as starting
...
\subsection{Use of the algorithm arguments}

The arguments mentioned in the preceding section can be used in the following way
...
\begin{enumerate}
\item First do an estimation with the default arguments
    for the algorithm.
    ...
    E.g., with one processor and
    for a model with $p=10$ parameters, you might use
    \sfn{nsub=1}, \sfn{n2start=2000}, \sfn{firstg=0.01}, \sfn{n3=5000}.
    For $p=20$ parameters, you could use
    \sfn{nsub=1}, \sfn{n2start=4000}, \sfn{firstg=0.01}, \sfn{n3=10000}.
    With multiple processes the value of \sfn{n2start} could be
    reduced almost proportionately to \sfn{1/nbrNodes}.

    Continue estimation with these algorithmic arguments using \sfn{prevAns},
...
more likely success, than just carrying on very long with just the
\sfn{prevAns} argument and further the default algorithmic arguments.


The following function is meant to
iterate the execution of \sfn{siena} until it has converged.
...
using the \texttt{prevAns} argument and trying out various
...
\item The model does not fit well in the sense that even with well-chosen
      ...
      For large degrees, the square root versions of degree effects
      may be better than the raw versions (e.g., \texttt{outActSqrt}
      instead of \texttt{outAct}; and for multivariate networks,
      \texttt{outActIntn} with internal effect parameter $p=2$).
      ...
      given in the start of the output file produced by \sfn{write\_report}:
...
Another trick that may \underline{sometimes (rarely!)} be tried is the following.
...
are the following.
Further details are in the help file for \textsf{siena}.
...
\begin{tabbing}
 ...
 \texttt{ans\$version }      \> the \textsf{RSiena} version\\
...
in the function \textsf{sienaToConvergence}
presented above.
...
in the call of \textsf{set\_algorithm\_saom}.

During the estimation process, if the graphical user interface
is used (the default \\
\texttt{batch = FALSE} in the call of \textsf{siena}),
...
\begin{enumerate}
  \item On the \R console.\\
        When the \textsf{sienaFit} object produced by \textsf{siena}
        is called \texttt{ans},
  ...
  \item The function \textsf{siena\_table}
        is available which serves a similar purpose,
        but not using \textsf{xtable}.
        It has the option \texttt{sig=TRUE} to show asterisks
        indicating significance of the parameter estimates.
        This function writes the table to a file;
        the default file name of the table produced is the name of the
        \sfn{sienaFit} object.
        The choice between \textsf{xtable.sienaFit} and \textsf{siena\_table}
        depends on the preference for the tables produced.

        For importing the results of \sfn{xtable} or \textsf{siena\_table} into
        MS-Word, the following steps can be used.

        Request \texttt{siena\_table(ans)} for some \sfn{sienaFit} object,
  ...
  \item The function \textsf{siena}
        writes an output file which is an ASCII (`text') file that can be
        read by any text editor.
        Its default name is the name of the data object
         to which \texttt{\_out.txt} is appended. A different name
        can be specified in the call of \textsf{\sfn{set\_output\_saom}}.
...
iterations of the model estimation (see Section~\ref{S_GMoM_siena}).
...
is implemented in the function \sfn{siena}.
ML estimation is done by the function \sfn{siena}, using a set of options
created by \sfn{set\_algorithm\_saom} with \sfn{maxlike = TRUE}.
Bayesian estimation is done by the function \sfn{multi\_siena},
but is as yet implemented only for multilevel network modeling
(Section~\ref{S_multi_siena}).


\subsection{Generalized Method of Moments estimation}
\label{S_GMoM}

\subsubsection{Using the function \sfn{siena}}
\label{S_GMoM_siena}
...
Estimation by the GMoM with the function \sfn{siena} is obtained
...
in the algorithm object created by the function \sfn{set\_algorithm\_saom}.
...
in the probability model using \sfn{set\_effect},
...
in phase 1 and phase 3 and (at least) one iteration of the function \textsf{siena} is used.

The following estimation steps are recommended to reach the convergence of the algorithm quickly.
\begin{enumerate}
	\item  Run the function \textsf{siena} with one or two sub-phases (i.e.,
...
\end{enumerate}
If convergence is not adequate, one runs \textsf{siena} repeatedly with \texttt{prevAns = TRUE}.
...
in  \sfn{set\_effect} with \texttt{type = "eval"}
...
ML estimation is done by the function \sfn{siena}, using a set of options
created by \sfn{set\_algorithm\_saom} with \texttt{maxlike = TRUE}.
Bayesian estimation is done by the function \sfn{multi\_siena}.
...
For ML estimation, an important argument for tuning the
algorithm is the so-called \textsf{multiplication factor},
given in \sfn{set\_algorithm\_saom} as the argument \texttt{mult}.
...
this is formulated there for \textsf{multi\_siena}, but applies
equally well for ML estimation using \textsf{siena}.
\medskip
...
\citet{Greenan2015}.
The probabilities of the first 7 steps are determined by the argument \texttt{prML}
in  \textsf{set\_algorithm\_saom}. The probability of  `Move' is 1 minus the sum
...
Another argument of the algorithm that sometimes needs tuning
(but less often than the multiplication factor)
is the \emph{Initial value of the gain parameter}.
...
For missing data in the dependent variables (not in the covariates!),
a different procedure is used than the one in the Methods of Moments
estimation procedure.
The missing data at the beginning of the wave are modeled as random
variables with a simple prior distribution, given by the
distribution of the non-missing entries for this wave.
(For Maximum Likelihood estimation, this is a Bayesian incursion
in an otherwise frequentist approach...) 
The missing data at the end of the wave are modeled
as random variables with the distribution implied
...
made in the \textsf{set\_algorithm\_saom} function by setting the
\textsf{cond} argument.
...
\textsf{condvarno} and \textsf{condname} arguments in this function.
...
by using the \textsf{set\_effect} function with the
...
\item
  ...
  multiple processes can speed up the estimation in \textsf{siena},
 % \sfn{sienacpp},
  and \textsf{test\_gof}.
...
\end{verbatim}
  ...
  There are some examples in the help page for \textsf{siena}.
\item For estimation by Method of Moments (MoM), in Phases 1 and 3
  the simulations are performed in parallel. In Phase~2,
  multiple (viz., \texttt{nbrNodes} which is an argument
  given in the call of \sfn{siena}) simulations are done with the same parameters,
...
\item For estimation by Maximum Likelihood (ML) and by \sfn{multi\_siena},
...
\item The arguments required to run all processes on one computer are fairly
  simple: in your call to \textsf{siena}, set \sfn{nbrNodes} to the number of
  processes and \sfn{useCluster} and \sfn{initC} to TRUE.

\item To use more than one machine is more complicated, but it can be done by
  using, in addition, the \sfn{clusterString} argument.  The computers need to
...
\item On Windows, sub processes are always started using \R scripts. On Linux
  ...
  code interface to \textsf{siena} to ask for the sub processes to be formed by
...
This can be checked this by running \textsf{siena} several times, with different
...
in \textsf{set\_algorithm\_saom}, so phases 1 and 2 are
skipped and the whole operation of \textsf{siena} takes considerably less time.
\medskip

The method for estimating derivatives is set
by the \sfn{findiff} parameter and the number of iterations in Phase~3
by the \sfn{n3} parameter, both in function \textsf{set\_algorithm\_saom}
...
When \textsf{siena} finds near-perfect collinearity at the end of the
...
`Wald test'\footnote{Abraham Wald (1902–1950)}
is the fancy name for the usual $t$-test
when based on Maximum Likelihood estimates;
or $F$-test for multivariate parameters.
For the \SAOM when the Method of Moments is used,
we use the term `Wald-type tests'.
These are based on the parameter estimates and their covariance matrix.
...
The function \sfn{test\_parameter} can be used for testing multiple
parameters simultaneously but, like in this example,
can be used also for a single parameter.
The well-known representation of $p$-values by asterisks
is available from
\begin{verbatim}
...
For testing  the null hypothesis that several parameters are 0,
the Wald-type test can also be applied.
The function \sfn{test\_parameter} produces the chi-squared value of the
...
(ego, alter, similarity) are zero, the command
\begin{verbatim}
...
(Here we use the term "$Z$-value"; elsewhere we have used the term "$t$-ratio";
for practical purposes these terms are equivalent here,
because we do as if the number of degrees of freedom is infinite, so
that the standard normal distribution and the $t$-distribution are the same.)

\subsubsection{Tests of linear combinations}

The principle of the Wald-type test is more general.
It can be used to test  null hypotheses that
can be represented as a linear constraint on $\theta$:
\[
H_0 : \ A\,\theta \,=\, 0 ,
\]
...
Consider the example that an evaluation and an endowment/maintenance function
...
\item maintenance of ties:\ evaluation parameter + maintenance parameter.
\end{itemize}
...
\textsf{siena\_table}). The name of the \texttt{sienaFit} object
was \texttt{G160\_net\_4}.

\begin{center}
% Table based on sienaFit object G160_net_4 , Thu Mar 12 11:38:05 2026
%Estimation date Tue Mar 10 21:24:41 2026
...
\hline
\rule{0pt}{2ex}\relax
outdegree (density)                            &  --2 & 984$^{\ast\ast\ast}$ & (0 & 093)\\
reciprocity                                    &    4 & 338$^{\ast\ast\ast}$ & (0 & 433)\\
reciprocity maintenance                        &  --1 & 941$^{\ast\ast}$     & (0 & 700)\\
GWESP I $\rightarrow$ K $\rightarrow$ J (69)   &    3 & 428$^{\ast\ast\ast}$ & (0 & 311)\\
GWESP I $\rightarrow$ K $\rightarrow$ J (69) maintenance &  --3 & 537$^{\ast\ast\ast}$ & (0 & 681)\\
rec.degree - activity                          &  --0 & 262$^{\ast\ast\ast}$ & (0 & 028)\\
sex alter                                      &  --0 & 110                  & (0 & 082)\\
sex ego                                        &    0 & 062                  & (0 & 088)\\
same sex                                       &    0 & 480$^{\ast\ast\ast}$ & (0 & 079)\\
...
\noindent
For creation of new ties, in  this model only the evaluation parameters
should be considered. For maintenance of existing ties, equation (\ref{fe_net})
says that the evaluation parameter and the maintenance parameter
should be added.
This leads to a contribution of  $4.338 -1.941 = 2.397$ for reciprocity
and $3.428 - 3.537= -0.109$ for transitivity (gwespFF). Are these significant?

This can be answered by using the method explained above with
a one-dimensional $ 1 \times  12$ matrix
\[
...
A[1,4] <- 1
A[1,5] <- 1
test_parameter(G160_net_4, tested=A)
\end{verbatim}
The result is that the standard error of the sum of the two
estimated parameters is $0.37$
and the $Z$ value (standard normal test statistic) is  $Z = 6.45$,
highly significant.
...
on maintenance and on creation of ties. In this case, the difference is --1.941,
with $t$-ratio $t = -1.941/0.700 = -2.77$, $p < 0.005$.
We may conclude that, in this model, the effect of reciprocation on
creation of ties is significantly (two-sided $p < 0.005$) larger than
on maintenance of ties; but the latter, on itself,
still is significant.

\subsubsection{Tests of equality of parameters}

The hypothesis that two parameters in one model have the same value
can also be tested using the principle of the multi-parameter Wald test.
This is implemented for this particular case by the
function \sfn{test\_parameter} with argument \texttt{method="same"}.
E.g., if \texttt{ans} is the estimated \texttt{sienaFit} object and
we wish to test the null hypothesis that parameters with index numbers
9 and~10 are equal, we use
\begin{verbatim}
test_parameter(ans, method="same", tested=9, tested2=10)
\end{verbatim}


\subsection{Score-type tests and one-step estimates}
\label{S_Scoretest}
...
This is done using the generalized Neyman-Rao
\footnote{Jerzy Neyman, 1894-1981;
Calyampudi Radhakrishna Rao, 1920-2023} score test
that is implemented for the Method of Moments estimation method
in \si\ as a part of function \textsf{siena}, following the methods
...
The function \textsf{set\_effect} is available to do this.
...
\end{verbatim}
Suppose that the effect of transitive ties has number~10 in the output;
then
\begin{verbatim}
...
It should be noted that using the \sfn{prevAns} option in \textsf{siena}
overrides the initial values in the effects object,
so that using \sfn{siena} for an effects object with
...
and therefore it often will be preferable to proceed as follows.
First update the initial values using \sfn{update\_theta}, then
set the hypothesized value by \sfn{set\_effect}, and then
carry out the estimation and score-type test by \sfn{siena}.
...
\subsubsection{One-step estimates}

Related to the score-type test are the so-called \emph{one-step estimates}.
These are `quick and easy' estimates which are not
...
only for parameter vectors where this parameter is fixed.
Often, when parameters are fixed, they are fixed to 0;
but other values could be used, and this would be preferable if they are closer
to an appropriate value for this parameter.

The one-step estimates are more reliable according as
the significance of the fixed-and-tested effects is smaller
and there is less collinearity
...
\end{verbatim}
Sometimes, e.g., if there are strong collinearities between the fixed
parameters, some or many of the one-step estimates are large (larger than 10
in absolute value). Then they are useless -- although they may be useful to indicate
which parameters are collinear with each other.

If the one-step estimates have reasonable values,
an alternative to the procedure mentioned above (leading to \texttt{myans1})
is to put the one-step estimates in the effects object.
E.g., if answer object \texttt{ans} was obtained using effects object \texttt{myeff}
and there was only one fixed-and-tested effect, with number~10, the
commands that could be given are
\begin{verbatim}
...
not problematic.
As mentioned above, the  one-step estimates can be a guide
to indicate which parameters are collinear.
...
This can be tested by the \sfn{test\_time} function,
...
or whatever.
The summary of \sfn{test\_time} gives information that can
...
At the \SI website there is a script \texttt{RscriptSienaTimeTest.r}
with an extensive example.
...
Simulations are produced by the \textsf{siena} function
...
the model specification in function \textsf{set\_algorithm\_saom}.
...
The principle is to use an `estimation' algorithm
\begin{verbatim}
alg_alg  <-  set_algorithm_saom(cond=FALSE,
                                nsub=0, n3=1000, simOnly=TRUE)
\end{verbatim}
(where a different value than 1000 for the number of runs may be used);
but this has to be further specified.

There are three ways to determine the parameter values that will be
used in the simulations. One is to use the parameter values that are
present in the effects object. Then we use
\begin{verbatim}
alg_alg  <-  set_algorithm_saom(cond=FALSE,
...
myeff
\end{verbatim}
and different values can be specified by the function \sfn{set\_effect}.

The second way to determine the parameter values for simulation
is that these are the parameters in an available estimated
model \texttt{ans}. This can be done using
\begin{verbatim}
alg_alg  <-  set_algorithm_saom(cond=FALSE,
               nsub=0, n3=1000, simOnly=TRUE)
...
\end{verbatim}
The value of \texttt{useStdInits} does not matter if the argument
\texttt{prevAns} is used for \sfn{siena}.

A third possibility is, for co-evolution models,
to use different estimated models for the various dependent
variables (networks, behavior). This can be achieved by using function
\sfn{updateTheta} to get the estimated parameters in the effects object
and then work with \texttt{useStdInits = FALSE}.
See the help page for \sfn{updateTheta}.
\medskip
...
object produced by \textsf{siena}.
...
The help page for \textsf{siena} contains an example for how to access the
...
that are also used for \sfn{test\_gof}.
...
of \rs, described in Section~\ref{S_cond}.
...
argument \texttt{thetaValues} for \textsf{siena}.
...
and the number of processes is \\
\texttt{nbrNodes} $ > 1$,
...
\SI website \\
\url{https://www.stats.ox.ac.uk/~snijders/siena/siena_scripts.htm} \\
and start reading and playing with them.
There is one script, \textsf{basicRSiena.r}, which is a very short
general introduction. Then there is a collection of five scripts:
\begin{enumerate}
  \item \textsf{Rscript01DataFormat.R}
  \item \textsf{RSienaSNADescriptives.R}
  \item \textsf{Rscript02SienaVariableFormat.R}
  \item \textsf{Rscript03SienaRunModel.R}
  \item \textsf{Rscript04SienaBehaviour.R}
\end{enumerate}
which together are meant as an introduction to get started.
As the names suggest, the second of these does not use \rs,
but package \textsf{sna}.
\bigskip

As a more general manner to find your own way,
...
\textsf{make\_data\_rsiena}, \textsf{make\_specification}, and \textsf{set\_algorithm\_saom},
...
Then let the program estimate the parameters, using function \textsf{siena}.
...
are the transitive triplets effect or the
gwesp (shortName \texttt{gwespFF}) effect, and any
...
Note that in Chapter~\ref{S_math} all shortNames are given within
parentheses, so that if you are looking for the definition
of a particular shortName, e.g., \texttt{gwespFF},
you can search in the manual for the string \texttt{(gwespFF)}.
By the way, it is much more convenient and energy-friendly
to download the manual on your own computer and read it there, then
to keep it for reading and searching at the internet.
...
in \citet*{SnijdersEA10b}, and in \\
\url{https://www.stats.ox.ac.uk/~snijders/siena/Siena_ModelSpec_s.pdf} .

When the distribution of the out-degrees is fitted poorly
(which can be inspected using the \textsf{test\_gof} function
...
\item Combining different groups
      ...
      and analyzing this by \textsf{siena}.
...
\item Analyzing the different networks separately, without any assumption
      ...
      using \textsf{meta\_siena}.
      This is explained in Section~\ref{S_Siena08}.
\item Combining different groups into one \emph{multi-group} data set
      as in option (2), but analyzing this by \textsf{multi\_siena}.
      This is explained in Section~\ref{S_multi_siena}.\\
      ...
      the parameter \texttt{random} in function \sfn{set\_effect}.
\end{enumerate}
...
then one might start by option (2), and use \textsf{test\_time} to test
...
to use \texttt{allowOnly = FALSE} in the call of \textsf{as\_dependent\_rsiena}.

\subsection{Function \textsf{make\_group\_rsiena}  }
\label{S_multigroup}

The function \textsf{make\_group\_rsiena} operates on a list
of \sfn{siena} data sets created by \textsf{make\_data\_rsiena},
...
For centered actor covariates, function \textsf{make\_group\_rsiena}
...
For actor covariates, function \textsf{make\_group\_rsiena} recalculates
...
can be used for the functions \sfn{as\_covariate\_rsiena} etc.
...
This is done by the function \textsf{make\_group\_rsiena} which creates
...
The function \textsf{meta\_siena} is a meta-analysis method for \si.
...
The function  \textsf{meta\_siena} takes as input the \textsf{sienaFit}
objects produced by separate runs of  \textsf{siena}.
...
variables must be renamed in the call of \textsf{make\_data\_rsiena};
...
then later are combined by \textsf{meta\_siena},
by using \texttt{allowOnly = FALSE} in the call of \textsf{as\_dependent\_rsiena},
as mentioned in Section~\ref{S_monotone}.
...
In terms of \citet{Viechtbauer2010}, \sfn{meta\_siena} follows a
...
Since this is not implemented in \sfn{meta\_siena}, this recommendation
...
\textsf{siena}, yielding the estimate $\hat\theta_j$,
...
For each of the parameters $\theta$, the function \textsf{meta\_siena}
...
by using \textsf{meta\_siena} and following it up by
...
\label{S_multi_siena}
  ...
  The function \textsf{multi\_siena} is for Bayesian estimation of one group or
  ...
  The difference is made by setting the argument \texttt{random} in
  the function \sfn{set\_effect}. The default is that only the out-degree (density)
  ...
  The operation of \textsf{multi\_siena} is explained in this section,
  ...
  This can be replaced by a run of \textsf{siena} outside of \textsf{multi\_siena},
  and giving the results of this run to \textsf{multi\_siena}
  ...
  to prevent the issues that arise with such variables in normal use of \textsf{siena}.\\
  ...
  in the call of \textsf{set\_algorithm\_saom} that created the algorithm object.
\end{enumerate}
  The function \textsf{multi\_siena} is time-consuming. When starting to use it, 
  it is advisable
...
\subsubsection{Which data sets to use for multi\_siena}
\label{S_sBData}

\textsf{multi\_siena} uses as data set a \sfn{sienaGroup} object with 2 or more groups.
The number of waves should be the same for all groups.

The following data configurations are not allowed for groups
included in \textsf{multi\_siena} estimation:
\begin{enumerate}
\item tie variables in two consecutive waves changing from
    structural zero (code 10) to 1;
\item tie variables in two consecutive waves changing from
...
\item and, for more than three consecutive waves, similar patterns
    with more NAs in between.
\end{enumerate}
To use data sets including such patterns for \textsf{multi\_siena},
...
In addition to what is or is not allowed as data sets for \textsf{multi\_siena},
...
for the estimation in \textsf{multi\_siena} of the parameters.
...
\textsf{multi\_siena} should be possible for groups as small as 5 actors.
...
the use of \sfn{siena} with the (default) Method of Moments, followed by a
meta-analysis by means of \sfn{meta\_siena} or \sfn{metafor}, may be preferable.

It is advisable to first do a multi-group analysis of the same model,
followed by a \textsf{test\_time}, to get an initial understanding of where problems
might occur. You may then later use the result for the \texttt{prevAns}
parameter of \textsf{multi\_siena}.


\subsubsection{Model specification}

Non-constant rate functions currently are not supported in \textsf{multi\_siena}.

The extra part of model specification for \textsf{multi\_siena},
compared to \textsf{siena},
is that it is required to specify which parameters are
randomly varying from group to group, and which are constant across groups.
The specification of constant vs.\ randomly varying
for the other parameters is done in the function
\textsf{set\_effect}, by its parameter \texttt{random}.
...
and input parameter \\
\texttt{priorRatesFromData} needs to be set to --1.
\medskip
...
\item[\emph{knowledge:}] is there prior knowledge about whether
   ...
   followed by a \textsf{test\_time}, and specify those effects as randomly
...
\item[\emph{misspecification:}] if it would be erroneously assumed
    that the effect is constant across groups, would this affect
...
\item[\emph{convergence:}] specifying effects as random will
    ...
    will have better convergence properties than constant parameters;
...
\item[\emph{amount of information:}] which specification will
     ...
     (Here finally we do have a provisional answer. Assuming that an effect is constant
     ...
     cf.\ Section~\ref{S_multi_sienaInterpret}.
     ...
     it is advisable to specify that they are constant.)
\end{description}


\subsubsection{How to enter your data in multi\_siena}

See the example at the bottom of the \textsf{multi\_siena} help page for how a
...
\textsf{as\_dependent\_rsiena}; see the help page for \textsf{sienaDependent}.
...
\subsubsection{How to choose the parameter settings for multi\_siena}
\label{S_multi_sienaParameters}

It is good to have an initial estimation as a multi-group estimation
using the Method of Moments, i.e., using \textsf{siena}, with
an algorithm having
...
This result of  \textsf{siena} can then be given as the \texttt{prevAns}
parameter to \textsf{multi\_siena} (see the help page).
Note that the parameter \texttt{firstg} corresponds to \texttt{initgainGlobal}
of \textsf{multi\_siena}, if no  \texttt{prevAns} is used.
...
\textsf{set\_algorithm\_saom}. Increasing these will for both of them lead to
...
random = TRUE
\end{verbatim}
in the call of \texttt{set\_effect}.
...
and $\kappa_0$.
These are specified for \textsf{multi\_siena} in \RS in the following way:
\begin{itemize}
  \item $\nu_0$ = \texttt{priorDf};
...
specified for \textsf{multi\_siena}:
\begin{itemize}
...
In the object produced by \textsf{multi\_siena}, let us call it \texttt{ans},
...
Assuming that \texttt{mydaya}  and \texttt{myeff} are,
respectively, a multigroup data set and an effects object,
...
S7 <- matrix(0, 7, 7)
diag(S7) <- 0.01
ans <- multi\_siena(mydata, myeff, priorMu = m7, priorSigma = S7, priorDf = 22, priorKappa = 0.01,
...
\subsubsection{Operation of \textsf{multi\_siena}}

In Section \ref{S_multi_sienaParameters} it was already suggested to
...
\textsf{multi\_siena}; see Section~\ref{S_multipleProcesses} where it is explained
that parallelization is different for \textsf{multi\_siena} than for MoM
estimation by \textsf{siena}.
...
and then combine the earlier with the later results using \sfn{multi\_glue}.
This is illustrated in the example on the help page.

During operation of \sfn{multi\_siena}, partial results of the function
...
These are \sfn{multi\_sienaFit} objects, and therefore can be used in the
...
\sfn{multi\_sienaFit} object (see \texttt{?print.sienaBayesFit}).
...
the \texttt{mult} parameter set in \sfn{set\_algorithm\_saom}.
...
Function \sfn{extract.multi\_siena} can be used to extract
...
Currently there is no good way in \sfn{multi\_siena} to use
...
\subsubsection{Interpreting results of multi\_siena}
\label{S_multi_sienaInterpret}
...
The function \textsf{siena\_table} can be used also for making
\texttt{html} or \LaTeX\ tables of the results for the global parameters;
here again, the parameter \texttt{nfirst} may be used.

The functions \sfn{test\_parameter} and \sfn{test\_parameter}
...
\sfn{multi\_sienaFit} object, as mentioned in the help page, may be useful.
...
The function \sfn{extract\_posteriorMeans} can be used to obtain
...
operation of \textsf{multi\_siena}') which groups are
...
It should be noted that the \texttt{.txt} files produced by \sfn{multi\_siena}
...
to specifying the effects as constant or varying across groups,
...
\subsubsection{Internal effect parameters}
\label{S_effpar}
...
These are set or modified by the \texttt{set\_effect} function, e.g.,
\begin{verbatim}
myeffects <- set_effect(myeffects, gwespFF, parameter=69)
\end{verbatim}
The value of the internal effect parameter is mirrored in the
effect name shown in \RS results; e.g., the effect mentioned here
will be represented as\\
\texttt{GWESP I->K->J (69)} \ ;\\
as another example, when the internal effect parameter refers to taking a square root,
this will be reflected by \texttt{sqrt} in the effect name.
...
which can be modified by the function \texttt{set\_effect(..., parameter=..., ...)}.
\iffalse
...
%       =  (n-2)b_0 - \big( x_{+i}+x_{+j} -x_{ij}-x_{ji}-2\sum_h x_{hi}x_{hj} \big)
 ...
 and endowment effects (using  \texttt{type=creation} and \texttt{type=endow}),
...
\end{equation}
 ...
 \item {\em out-degree related popularity more than $p$ effect} \texttt{(outPopMore)}, defined by
  the sum of the difference between the out-degrees  of the others to whom $i$ is tied,
  and the internal effect parameter $p$, truncated to positive values: \\
 $s^{\rm net}_{i\vit}(x) =  \sum_j x_{ij}\, \max\{ x_{j+} - p, 0 \}  $;


 \item {\em out-degree related popularity (sqrt) more than $p$ effect}
 \texttt{(outPopSqrtMore)}, defined by
  the sum of the difference between the square roots of the
  out-degrees  of the others to whom $i$ is tied,
  and the internal effect parameter $p$, truncated to positive values: \\
 $s^{\rm net}_{i\vit}(x) =  \sum_j x_{ij}\, \max\{ \sqrt{x_{j+}} - \sqrt{p}, 0 \}  $;


 \item {\em out-degree related popularity more than $p$ effect} \texttt{(outPopThreshold)}, defined by
  the number of others to whom $i$ is tied who have outdegree larger than the
  internal effect parameter $p$: \\
 $s^{\rm net}_{i\vit}(x) =  \sum_j x_{ij}\, I\{ x_{j+} > p \}  $;
...
\item \emph{in-isolate Outdegree effect}, (\texttt{inIsDegree}), the
 ...
 \item[{\hspace*{-1ex}$\bigodot$}]
The following effects indicated by the suffix \texttt{\_ego} are meant mainly for interactions.
Recall that for interaction effects, the internal effect parameters
should match; see Section~\ref{intint}.
They are elementary effects (see Section~\ref {S_elementary}) and have
\texttt{interactionType="ego"},
which means they can be used in interactions  and in endowment and creation functions.
 ...
 \item {\em out-degree related activity more than $p$ ego effect} \texttt{(outActMore\_ego)},
 	 defined as the elementary effect of outdegree activity
 for the positive difference of outdegrees and the internal effect parameter $p$:\\
 $ f^{\rm net,\, el}_{i\vit}(x)  =  \max \{  x_{i+} - p, \, 0 \} $ ;\\

 \item {\em out-degree related activity more than $p$ (sqrt) ego effect} \texttt{(outActSqrtMore\_ego)},
 	 defined as the elementary effect of outdegree activity
 for the positive difference of the square roots of the outdegrees and the internal effect parameter $p$:\\
 $ f^{\rm net,\, el}_{i\vit}(x)  =  \max \{  \sqrt{x_{i+}} -\sqrt{p} ,\, 0 \} $ ;\\

 \item {\em out-degree related activity with threshold $p$ ego effect} \texttt{(outMore\_ego)},
 	 defined as the elementary effect of outdegree activity
 for  outdegrees larger than the internal effect parameter $p$:\\
 $ f^{\rm net,\, el}_{i\vit}(x)  =  I \{  x_{i+} > p \} $ ;\\
 this is the elementary-effect version of \texttt{outMore};
...
\paragraph{Dyadic covariate effects}
...
\paragraph{Monadic covariate effects}
...
\setcounter{enumi}{\value{savenumi}}
 ...
 for using no centering, work with the effect \texttt{(altX\_nc)} instead;
...
defined by \\
 ...
 for using no centering, work with the effect \texttt{(egoX\_nc)} instead;
...
\item[$\bigodot$]
  the analogue of the \texttt{sameXOutAct} effect (see below)
  (restricting indegree popularity to only within $V$-groups)
  is the interaction of \texttt{(sameXInPop)} with \texttt{sameX},
  for the same covariate:\\
 $s^{\rm net}_{i\vit}(x) = \sum_j x_{ij}\,I\{v_i = v_j \} \sum_h x_{hj}\, I\{v_i = v_h \} $;\\
...
\end{minipage}
 ...
 here $\overline {v}$ is the mean of the values $v_{h}$ for all actors $h$
 ...
 \item {\em group total} \texttt{(totGroupEgoX)}, defined by ego's value multiplied by the
 sum of the covariate for this period,  \\
 $s^{\rm beh}_{i\vit}(x, z) =  \sum_j x_{ij} \, \sum_h{v_h}  $;\\
 here $\sum_h{v_h} $ is the sum of the values $v_{h}$ for all actors $h$
 in the actor set: for changing covariates, the total of the wave at the
 start of the period; for behavioral dependent variables,
 the current total.
...
\[
 s^{\rm net}_{i\vit}(x) = \sum_j x_{ij} (x_{j+} - x_{ji}) \breve v_j^{(-i)} \hfill
                          (\text{parameter 2}) \hfill
\]
      where $\breve v_j$ and $\breve v_j^{(-i)}$ are as above; for
      using non-centered covariate and behavior values use \texttt{totDist2\_nc}.
...
where $\bar v$ is (again) the global mean of $V$.
 ...
 \item two variations of the
   \emph{GWDSP (geometrically weighted dyadwise shared partners)} covariate effects:
    {\em GWDSPFF covariate} \texttt{(gwdspFFX)} and
    {\em GWDSPFB covariate} \texttt{(gwdspFBX)}.
   For each distance-2 alter $h$, its covariate value $v_h$ is weighted by the
   geometrically weighted number of shared partners linking $i$ and $h$,
      \begin{equation}
      s^{\rm net}_{i\vit}(x, v, \alpha) \,=\,
                  \sum_{h;\; h \neq i}^n \, v_{h}\,
                  e^{\alpha}\,\Big\{1\,-\,\big(1 - e^{-\alpha}\big)^{
                        \sum_{j} x_{ij}x_{jh} }  \Big\}
            \, \
      \end{equation}
      and
      \begin{equation}
      s^{\rm net}_{i\vit}(x, v, \alpha) \,=\,
                  \sum_{h;\; h \neq i}^n  \, v_{h}\,
                  e^{\alpha}\,\Big\{1\,-\,\big(1-e^{-\alpha}\big)^{
                        \sum_{j} x_{ij}x_{hj} }  \Big\}
            \, \ ;
      \end{equation}
   the FF variant uses the two-path $i \rightarrow j \rightarrow h$, the FB variant
   the shared partner $i \rightarrow j \leftarrow h$. On its own this is only the
   distance-2 kernel (no $v_i$ factor); in an interaction with \texttt{egoX} it
   becomes the full indirect-homophily statistic $v_i \sum_h v_h\, w(\cdot)$ -
   the selection analogue of the influence effect \texttt{totGwdspFBAlt}. The
   weighting damps redundant paths to the same distance-2 alter: with strong
   weighting ($\alpha \to 0$) it converges to a social-circle effect in which each
   distance-2 alter's covariate value counts once, regardless of how many shared
   partners connect it to $i$. For a non-centered covariate use
   \texttt{(gwdspFFX\_nc)} / \texttt{(gwdspFBX\_nc)} instead;

\item Two variations of the
   \emph{GWDIST2 (geometrically weighted distance 2) covariate} effects:
    {\em GWDIST2 covariate FF} \texttt{(gwdist2XFF\_nc)} and
    {\em GWDIST2 covariate FB} \texttt{(gwdist2XFB\_nc)}.
   In contrast to the GWDSP variants, the geometric weighting here applies to the
   distance-2 neighbourhood itself, penalizing the accumulation of covariate values
   reached through the same intermediary $j$,
      \begin{equation}
      s^{\rm net}_{i\vit}(x, v, \alpha) \,=\,
                  \sum_{j}^n \, x_{ij}\,
                  e^{\alpha}\,\Big\{1\,-\,\big(1 - e^{-\alpha}\big)^{
      ...
      s^{\rm net}_{i\vit}(x, v, \alpha) \,=\,
            ...
            \, \;
      \end{equation}
   the FF variant sums $v_h$ over $j$'s out-neighbours ($i \rightarrow j \rightarrow
   h$), the FB variant over $j$'s in-neighbours ($i \rightarrow j \leftarrow h$).
   Whereas GWDSP damps multiple paths to the same alter, GWDIST2 models exposure
   saturation through a single gateway $j$: with strong weighting ($\alpha \to 0$)
   only \emph{whether} a direct alter $j$ reaches some distance-2 alter with $v_h > 0$ matters, not how many or how much (for
   binary $V$ it then counts such direct alters $j$). The sum behind $gw(\cdot)$
   must be non-negative, so only the non-centered form exists; in an interaction
   with \texttt{egoX\_nc} it models indirect homophily by relational mimesis, where
   having ties to the same alters as others with high covariate values matters more
   than the overall covariate volume of ego and the indirect alters.
...
\]
    ...
    this is a $V$-restricted version of indegree popularity; for 
    using this without centering a behavior variable $z$, use
     \texttt{totInDist2\_nc} instead.
...
\paragraph{Monadic double covariate effects}
...
\setcounter{enumi}{\value{savenumi}}
 ...
 This effect is implemented only for $U$ and $V$ that are non-centered categorical
  ...
  see the help page for \sfn{as\_covariate\_rsiena} and \sfn{as\_covariate\_rsiena}.
...
\citep[see Section~\ref{S_modeltype_nd} and][]{SnijdersPickup17},
...
\smallskip
\item
 {\em any agreement along W leading to X}, ($X$: from $W$ any agreement)  \texttt{(fromAny)} \\[0.2em] % \texttt{(fromWAgree)}\\
...
\put{\large$\bullet$} at  3 2.732
\put{\small$\blacklozenge$} at  2.3 1.85 % k
\put{$i$} at 2 0
\put{$j$} at 4 0
\put{$h$} at 3 3.4
\put{$k$} at 2.65 1.9
\put{{\small $W$}} at 2.25 2.55
\put{{\small $W$}} at 1.8 1.5
%\put{{\small $X$}} at 3.7 2.4
\put{{\small $X$}} at 3   0.3
\arrow <2mm> [.2,.6]  from 2.2 0.7 to 3.8 0.7 % i to j
\arrow <2mm> [.2,.6]  from 2 0.9 to 2.2 1.65 % i to k
\arrow <2mm> [.2,.6]  from 2.85 2.6 to 2.4 2.1 % h to k
%\arrow <2mm> [.2,.6]  from  3.2 2.8 to 4.5 2.8
\endpicture
\end{center}
\vfill
...
\put{{\small $X$}} at 1.5 1.3
\put{{\small $W$}} at 2.5 1.3
\put{{\small $W$}} at 3.5 1.3
\put{{\small $X$}} at 4.5 1.3
\arrow <2mm> [.2,.6]  from 2.2 1 to 2.8 1
\arrow <2mm> [.2,.6]  from 1.8 1 to 1.2 1
...
\noindent
The effects for a dependent network $X$, with a co-evolving
network $W$, and a dyadic covariate $U_{ij}$ are the following.

For these effects the other network $W$ is given as \texttt{covar1},
and the dyadic covariate $U$ as \texttt{covar2}.
...
\begin{minipage}[t]{.7\textwidth}
 ...
 outgoing $X$-choices as those to whom they have a $W$-tie,
 weighted by $U$.\\
 Here $W$ is \texttt{covar1} and $U$ is \texttt{covar2}.\\
...
For these effects the other network $W$ is given as \texttt{covar1},
and the covariate $V$ as \texttt{covar2}.
...
Here again, the other network $W$ is given as \texttt{covar1},
and the covariate $V$ as \texttt{covar2}.
...
($X$: from $W$ agr.\ $\times$ same $V$)  \texttt{(covNetNet)},\\[0.2em]
 $s^{\rm net}_{i\vit}(x) = \sum_{j \neq h}
  ...
  (specified with \texttt{covar1} = $W$, \texttt{covar2} = $V$)\\[0.2em]
 this refers to agreement of actors with respect to their $W$-choices
...
\begin{equation}
  \check v_j^W  = \left\{\begin{array}{ll} \displaystyle
         \frac{\sum_{h \neq i} w_{hj}\,v_h}{x_{+j} - x_{ij}}  &
                                       \text{ if } x_{+j} > 0     \\
         \bar v                                &  \text{ if } x_{+j} = 0  ,
  \end{array}   \right.            \label{alt_inav2w}
\end{equation}
where $\bar v$ is (again) the global mean of $V$.
...
\paragraph{Effects for three networks}
...
\]
    $W$ is given as \texttt{covar1}, $Z$ as \texttt{covar2}.
...
To define rate effects by the function \sfn{set\_effect} or \textsf{set\_effect},
...
reciprocated degrees.
 ...
 \emph{These effects work properly only for non-conditional estimation
    (set \texttt{cond = FALSE} in \sfn{set\_algorithm\_saom}).  }
\end{enumerate}

When rate effects are included, sometimes it can be helpful to specify
a smaller value for the initial gain parameter in \sfn{set\_algorithm\_saom};
...
output file produced by \sfn{write\_report} under the heading
...
\begin{startenum}
 ...
 by default, this centering is done by the grand mean in the data;
 for using the contemporaneous mean, work with the effect
 \texttt{(quad\_cc)} instead; for using no centering, work with the effect
 \texttt{(quad\_nc)} instead;
 ...
 \medskip \label{avGroupNote}
 \noindent \emph{Ego exclusion (\texttt{\_noEgo}) and non-centered (\texttt{\_nc}) variants.}
 Each of the following six effects -- \texttt{avGroup}, \texttt{totGroup},
 \texttt{indegAvGroup}, \texttt{indegTotGroup}, \texttt{outdegAvGroup}, and
 \texttt{outdegTotGroup} -- has two further variants, which can also be combined
 (e.g.\ \texttt{avGroup\_nc\_noEgo}).:
 \begin{itemize}
 \item \texttt{..\_noEgo}: ego is excluded from the group of actors $h$ that the statistic is
 computed over, e.g.\ for \texttt{avGroup\_noEgo}. Ego's behavior is compared to
 the rest of the group, not including ego's own value. This does not make much of
 a difference if the (network) group size is relatively large. By default (no suffix), 
 ego is  included among the actors $h$, as in the formulas given below.
 \item \texttt{..\_nc}: the non-centered values of $Z$ are used in place of the values
 centered around $m_Z$. Unlike the \texttt{\_nc} suffix elsewhere in this manual (e.g.\
 \texttt{quad\_nc}), this also disables the $p$-recentering described below entirely:
 $c_p$ is not applied, regardless of the value of $p$. This is because, for non-centered
 values, $p$-recentering and the choice of representation are not independent adjustments;
 supplying both would not have an unambiguous combined meaning.
 \end{itemize}
 ...
 By default, ego is included among the actors $h$ summed over; for the
 variant excluding ego from its own reference group
 (\texttt{avGroup\_noEgo}), see the note abive for this effect family.
 For using non-centered values of $Z$, with no $p$-recentering applied at
 all, use \texttt{avGroup\_nc} instead -- see the note above.

 \item {\em group total} \texttt{(totGroup)}, defined by ego's value multiplied by the
sum of \underline{all} values $z_h$ for this period,  \\
$s^{\rm beh}_{i\vit}(x, z) =  z_{i} \left( \sum_{h} z_h - n\, c_p \right) $;\\
here $\sum_{h} z_h$ is the sum of the values $z_h$ for all actors $h$
in the group, $n$ is the number of actors in the group, and $c_p$ is a centering constant
depending on the internal effect parameter $p$, defined as for the \texttt{avGroup} effect
above. The factor $n$ keeps the recentering on the same scale as the sum: since $c_p$ is a
per-actor (mean-scale) quantity, e.g. with $p$ close to the median of $Z$, it must be scaled up by
group size $n$ to remain a meaningful adjustment to a group \emph{total} rather than a group
\emph{average}.
...
To specify such effects in \RS using, e.g., function \sfn{set\_effect},
...
in the argument \sfn{depvar}
as well as the network in the argument \sfn{covar1}.
For example,
{\small
\begin{verbatim}
...
% where again $I\{A\}$ denotes the indicator function of the condition $A$;
 ...
 \item
 {\em average popularity alter effect} \texttt{(totPopAlt)}, defined by the
 sum of in-degree of the other actors $j$ to whom $i$ is tied,\\
 $s^{\rm beh}_{i\vit}(x, z) =   z_i \, \sum_j x_{ij} x_{+j}  $;\\
 this effect may be useful, e.g., as a control effect for the total
 similarity $\times$ popularity alter effect and in distance 2 influence effects;
 for using no centering, work with the effect
 \texttt{(totPopAlt\_nc)} instead;

  \item
 {\em total popularity alter effect} \texttt{(popAlt)}, defined by the
 ...
 for using no centering, work with the effect
 \texttt{(popAlt\_nc)} instead;

 \item
 {\em total activity alter effect} \texttt{(totActAlt)}, defined by the
 sum of out-degrees of the other actors $j$ to whom $i$ is tied,\\
 $s^{\rm beh}_{i\vit}(x, z) =   z_i \, \sum_j x_{ij} x_{j+}  $;\\
 this effect may be useful, e.g., as a control effect for the total
 alter at distance 2 influence effects; for using
 no centering, work with the effect
 \texttt{(totActAlt\_nc)} instead;

  \item
 {\em average acvtivity alter effect} \texttt{(actAlt)}, defined by the
 average out-degrees of the other actors $j$ to whom $i$ is tied,\\
 $s^{\rm beh}_{i\vit}(x, z) =   z_i\,x_{i+}^{-1} \, \sum_j x_{ij} x_{j+}  $;\\
 (and 0 if $x_{i+} = 0$) ;\\
 this effect may be useful, e.g., as a control effect for the average
 alter at distance 2 influence effects; for using
 no centering, work with the effect
 \texttt{(actAlt\_nc)} instead;

 \item two \emph{total GWDSP (geometrically weighted dyadwise shared partners)} effects:
    {\em total GWDSPFF} \texttt{(totGwdspFF)} and
    {\em total GWDSPFB} \texttt{(totGwdspFB)}.
   They are defined by the geometrically weighted 
   number of shared partners (two-paths $i \rightarrow j \rightarrow h$ for FF, 
   shared in-partners $i \rightarrow j \leftarrow h$ for FB) linking $i$ to the other 
   actors $h$,
      \begin{equation}
      s^{\rm beh}_{i\vit}(x, z, \alpha) \,=\,
            z_{i} \,
                  \sum_{h;\; h \neq i}^n
                  e^{\alpha}\,\Big\{1\,-\,\big(1 - e^{-\alpha}\big)^{
                        \sum_{j} x_{ij}x_{jh} }  \Big\}
      ...
      s^{\rm beh}_{i\vit}(x, z, \alpha) \,=\,
            ...
            \, \ ;
      \end{equation}
   These can be useful structural controls for the corresponding indirect influence
   effects \texttt{totGwdspFFAlt} / \texttt{totGwdspFBAlt} below. Ego's behaviour 
   weighted can depend on how structurally embedded ego is with diminishing returns
   for a large number of paths two an indirect alter. For using no 
   centering of $z_i$, work with the effect \texttt{(totGwdspFF\_nc)} 
   respectively \texttt{(totGwdspFB\_nc)} instead;
 ...
 \item {\em average similarity $\times$ variance alter effect} \texttt{(avSimVarAlt)},
 defined by the average of centered similarity scores ${\rm sim}^z_{ij}$ between $i$
 and the other actors $j$ to whom he is tied, multiplied the centered variance of the behavior of his alters, \\
 $s^{\rm beh}_{i\vit}(x, z) = x_{i+}^{-1} \,
        \sum_j x_{ij}  ({\rm sim}^z_{ij} - \widehat{{\rm sim}^z}) \times \big(x_{i+}^{-1}\,\sum_j (x_{ij} z_j)^2 - x_{i+}^{-2}\,
       (\sum_j x_{ij}z_j)^2 - \widehat{{\rm var}^z} \big) $\\
 (and $0$ if $x_{i+} = 0$) ; \\
where $\widehat{{\rm var}^z}$ is the variance of the observed behavior values in all but the final wave; \\
 ...
 note that the mean behavior by default is the empirical grand mean in
 the data; for using the contemporaneous mean, work with
 \texttt{(avAlt\_cc)} instead;

 \item {\em total alter effect} \texttt{(totAlt)},
 defined by  $i$'s
 behavior multiplied by the sum of behavior of his alters, \\
 $s^{\rm beh}_{i\vit}(x, z) =  z_i \big( \sum_j x_{ij}\, z_j \big) ;  $\\
 also here a contemporaneously centered variant \texttt{(totAlt\_cc)} exists;
...
(cf.\ (\ref{alt_av3})), \\
 $s^{\rm beh}_{i\vit}(x, z) =  z_i \big( \sum_j x_{ij}\, \check z_j^{(-i)} \big)
                                / \big (\sum_j x_{ij}\big)  $\\[0.5ex]
 (and the mean behavior, i.e. $0$, if the ratio is 0/0) ; for using
 no centering, work with the effect
 \texttt{(avInAltDist2\_nc)} instead;
 ...
 excluding the contribution from a tie $i \rightarrow j$ (if any),\\
 $s^{\rm beh}_{i\vit}(x, z) = z_i \sum_j x_{ij}\, \sum_{h \neq i} x_{hj}\,z_h =
         z_i \sum_j x_{ij}\, (x_{+j}- x_{ij}) \, \check z_j^{(-i)}  $; for using
 no centering, work with the effect
 \texttt{(totInAltDist2\_nc)} instead;
 ...
 (and the mean behavior, i.e. $0$, if the ratio is 0/0) ; for using
 no centering, work with the effect
 \texttt{(avTInAltDist2\_nc)} instead;
 ...
 $s^{\rm beh}_{i\vit}(x, z) =  z_i \big( \sum_j x_{ij}\, \check z_j^{(-i)} \big) $;
 for using no centering, work with the effect
 \texttt{(totAInAltDist2\_nc)} instead;

 \item two variations of the
   \emph{GWDSP (geometrically weighted dyadwise shared partners) alter} effects:
    {\em total GWDSPFF alter} \texttt{(totGwdspFFAlt)} and
    {\em total GWDSPFB alter} \texttt{(totGwdspFBAlt)}.
   These are geometrically weighted versions of \texttt{totAltDist2} and
   \texttt{totInAltDist2}, respectively, penalizing additional paths between ego
   and his distance 2 alters defined by
      \begin{equation}
      s^{\rm beh}_{i\vit}(x, z, \alpha) \,=\,
            z_{i} \,
                  \sum_{h;\; h \neq i}^n \, z_{h}
                  e^{\alpha}\,\Big\{1\,-\,\big(1 - e^{-\alpha}\big)^{
                        \sum_{j=1} x_{ij}x_{jh} }  \Big\}
            \, \
      \end{equation}
      and
      \begin{equation}
      s^{\rm beh}_{i\vit}(x, z, \alpha) \,=\,
            z_{i} \,
                  \sum_{h;\; h \neq i}^n  \, z_{h}
                  e^{\alpha}\,\Big\{1\,-\,\big(1-e^{-\alpha}\big)^{
                        \sum_{j=1} x_{ij}x_{hj} }  \Big\}
            \, \ ;
      \end{equation}
in contrast to the next effect, this weighting scheme dampens structural path 
redundancy to a single actor. For $\alpha \to 0$, it converges towards a pure
social circle effect, where the number of two-paths
connecting $i$ to his distance 2 alters  (dyadwise shared partners) is ignored 
while each distance 2 alters behavior is counted once fully.
for using no centering, work with the effect
\texttt{(totGwdspFFAlt\_nc)} respectively
\texttt{(totGwdspFBAlt\_nc)} instead;

\item Two variations of the
   \emph{GWDIST2 (geometrically weighted distance 2) alter} effects:
    {\em total GWDIST2 alter FF} \texttt{(totGwdist2FFAlt\_nc)} and
    {\em total GWDIST2 alter FB} \texttt{(totGwdist2FBAlt\_nc)}.
   In contrast to the GWDSP variants, these geometric weightings apply directly to the 
   distance 2 neighborhood itself, penalizing the accumulation of additional distance 2 
   alters' behavior exposed through the same direct intermediary. They are defined by
      \begin{equation}
      s^{\rm beh}_{i\vit}(x, z, \alpha) \,=\,
            z_{i} \,
                  \sum_{j=1}^n \, x_{ij}
      ...
      s^{\rm beh}_{i\vit}(x, z, \alpha) \,=\,
            ...
            \, \;
      \end{equation}
while GWDSP dampens multiple paths to the same source, GWDIST2 models exposure 
saturation via a single gateway. With strong weighting ($\alpha \to 0$), this effect 
converges to a pure similarity effect, as only the presence of a behaviorally 
active distance 2 alter via a direct neighbor matters, while the total volume 
of such alters reached through that neighbor is discounted. These specific statistics 
have their most natural count-like interpretation for binary behavior variables. 
For non-negative valued behavior, the statistic remains coherent, but its interpretation 
shifts from counting configuration multiplicity to aggregating behavioral intensity with 
geometric decay. In symmetric networks, the FF variant \texttt{(totGwdist2FFAlt\_nc)} 
represents a geometrically weighted structural equivalence influence effect. There is no 
centered version of these GWDIST2 effects, as the weighting scheme requires non-negative 
values for the behavior kernel $\sum_{h=1;\; h \neq i} x_{jh} z_{h}$ (or its incoming 
equivalent).
 ...
 \item {\em variance alter effect} \texttt{(varAlt)},
 defined by  $i$'s
 behavior multiplied by the centered variance of the behavior of his alters, \\
 $s^{\rm beh}_{i\vit}(x, z) =  z_i \big(x_{i+}^{-1}\sum_j (x_{ij} z_j)^2 - x_{i+}^{-1}
       (\sum_j x_{ij}z_j)^2 - \widehat{{\rm var}^z} \big) $\\
 (and $0$ if $x_{i+} = 0$) ; \\
where $\widehat{{\rm var}^z}$ is the variance of the observed behavior values
in all but the final wave; \\
 ...
 $s^{\rm beh}_{i\vit}(x, z) =  x_{+i} \, x_{i+}^{-1}
      \sum_j x_{ij} ({\rm sim}^z_{ij} - \widehat{{\rm sim}^z}) $;\\
 (and 0 if $x_{i+} = 0$) ;\\
 because of collinearity, under the Method of Moments this cannot be estimated
 together with the average similarity $\times$ popularity alter effect;

 \medskip
 \noindent \emph{Ego exclusion and non-centered variants.} The following four 
 effects are weighted versions of the group effects. The note about
 alternative effect versions \texttt{\_noEgo} and \texttt{\_nc} (p.~\pageref{avGroupNote}) 
 applies here as well.

 \item {\em indegree-weighted group average} \texttt{(indegAvGroup)}, defined
 by ego's value multiplied by the  indegree-weighted average of all values
 $z_h$ for this period,  \\
 $s^{\rm beh}_{i\vit}(x, z) =  z_{i} \left( \frac{ \sum_{h} x_{+h} z_{h} }
      {\sum_{h} x_{+h}  } - c_p \right) $;\\
 here $x_{+h}$ is the indegree of actor $h$ in the specified network, and
 $c_p$ is a centering constant as for \texttt{avGroup}.

 \item {\em indegree-weighted group total} \texttt{(indegTotGroup)},
 defined by ego's value multiplied by the indegree-weighted sum of all values
 $z_h$ for this period,  \\
 $s^{\rm beh}_{i\vit}(x, z) =  z_{i}
 \left( \sum_{h} x_{+h} z_{h} - n\, c_p \right) $;\\
 here $x_{+h}$ is the indegree of actor $h$ in the specified network,
 $n$ is the number of actors in the group, and $c_p$ is a centering constant
 as for \texttt{avGroup}.

 \item {\em outdegree-weighted group average} \texttt{(outdegAvGroup)}, defined
 by ego's value multiplied by the outdegree-weighted average of all values
 $z_h$ for this period,  \\
 $s^{\rm beh}_{i\vit}(x, z) =  z_{i} \left( \frac{ \sum_{h} x_{h+} z_{h} }
      {\sum_{h} x_{h+}  } - c_p \right) $;\\
 here $x_{h+}$ is the outdegree of actor $h$ in the specified network, and
 $c_p$ is a centering constant as for \texttt{avGroup}.

 \item {\em outdegree-weighted group total} \texttt{(outdegTotGroup)},
 defined by ego's value multiplied by the outdegree-weighted sum of all values
 $z_h$ for this period,  \\
 $s^{\rm beh}_{i\vit}(x, z) =  z_{i}
 \left( \sum_{h} x_{h+} z_{h} - n\, c_p \right) $;\\
 here $x_{h+}$ is the outdegree of actor $h$ in the specified network,
 $n$ is the number of actors in the group, and $c_p$ is a centering constant
 as for \texttt{avGroup}.
...
\paragraph{Effects of multiple networks}
...
$X_1$ is given as \texttt{covar1}, $X_2$ as \texttt{covar2}.
...
% $s^{\rm beh}_{i\vit}(x, z) = z_i \sum_j x_{1ij}\,x_{1ji}\,x_{2ij}\,x_{2ji} \,$.
 ...
 \item {\em outdegree mixed popularity effect} \texttt{(outdegMixedPop)},
 defined by $i$'s behavior multiplied by the sum of the $X_2$-indegrees
 of ego's  $X_1$-out-alters :
 \\
 $s^{\rm beh}_{i\vit}(x, z) = z_i \, \sum_j x_{1ij}\,x_{2+j} \,$;\\[0.2em]

 \item {\em indegree mixed popularity effect} \texttt{(indegMixedPop)},
 defined by $i$'s behavior multiplied by the sum of the $X_2$-indegrees
 of ego's  $X_1$-in-alters :
 \\
 $s^{\rm beh}_{i\vit}(x, z) = z_i \, \sum_j x_{1ji}\,x_{2+j} \,$;\\[0.2em]
...
\item \emph{reciprocal alter's covariate average } effect on behavior $z$
 \texttt{(avXRecAlt)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the average of the covariate values $v_j$
      for those to whom $i$ has a reciprocal tie:
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,  \displaystyle
            \frac{\sum_j x_{ij}\,x_{ji} \, v_j }{\sum_j x_{ij}\,x_{ji} }$,\\
      where 0/0 is defined as 0;

\item \emph{reciprocal alter's covariate total } effect on behavior $z$
 \texttt{(totXRecAlt)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the sum of the covariate values $v_j$
      for those to whom $i$ has a reciprocal tie:
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,
            \sum_j x_{ij}\,x_{ji} \, v_j $; \\
...
\item \emph{alter's same covariate average } effect on behavior $z$
 \texttt{(avSameXAlt)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the proportion of those to whom $i$ has a tie
      who also have the same covariate value:
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,  \displaystyle
            \frac{\sum_j x_{ij} \, I\{v_j\,=\,v_i\} }{\sum_j x_{ij} }$,\\
      where 0/0 is defined as 0;

\item \emph{alter's same covariate total } effect on behavior $z$
 \texttt{(totSameXAlt)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the number of actors $j$ to whom $i$ has a  tie
      and who also have the same covariate value:\\
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,
            \sum_j x_{ij} \, I\{v_j\,=\,v_i\} $; \\



\item \emph{in-alter's same covariate average } effect on behavior $z$
 \texttt{(avSameXInAlt)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the proportion of those from whom $i$ has an incoming tie
      who also have the same covariate value:
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,  \displaystyle
            \frac{\sum_j x_{ji} \, I\{v_j\,=\,v_i\} }{\sum_j x_{ji} }$,\\
      where 0/0 is defined as 0;

\item \emph{in-alter's same covariate total } effect on behavior $z$
 \texttt{(totSameXInAlt)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the number of actors $j$ from whom $i$ has an incoming tie
      and who also have the same covariate value:\\
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,
            \sum_j x_{ji} \, I\{v_j\,=\,v_i\} $; \\




\item \emph{reciprocal alter's same covariate average } effect on behavior $z$
 \texttt{(avSameXRecAlt)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the proportion of those to whom $i$ has a reciprocal tie
      who also have the same covariate value:\\[0.4em]
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,  \displaystyle
            \frac{\sum_j x_{ij}\,x_{ji} \, I\{v_j\,=\,v_i\} }{\sum_j x_{ij}\,x_{ji} }$,\\
      where 0/0 is defined as 0;

\item \emph{reciprocal alter's same covariate total } effect on behavior $z$
 \texttt{(totSameXRecAlt)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the number of actors $j$ to whom $i$ has a reciprocal tie
      and who also have the same covariate value:\\
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,
            \sum_j x_{ij}\,x_{ji} \, I\{v_j\,=\,v_i\} $; \\


\item \emph{average alter for same covariate } effect on behavior $z$
 \texttt{(avAltSameX)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the average behavior of those to whom $i$ has a tie
      and who also have the same covariate value:\\[0.6em]
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,  \displaystyle
            \frac{\sum_j x_{ij} \, I\{v_j\,=\,v_i\}\, z_j }{\sum_j x_{ij}\, I\{v_j\,=\,v_i\} }$,\\
      where 0/0 is defined as 0;

\item \emph{total alter for same covariate  } effect on behavior $z$
 \texttt{(totAltSameX)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the sum of behavior of those to whom $i$ has a tie
      and who also have the same covariate value:\\
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,
            \sum_j x_{ij}\, I\{v_j\,=\,v_i\} \, z_j $; \\

\item \emph{average reciprocated alter for same covariate } effect on behavior $z$
 \texttt{(avRecAltSameX)},\\[0.3em]
      defined as the product of $i$'s behavior $z_i$ and
      the average behavior of those to whom $i$ has a  reciprocated tie
      and who also have the same covariate value:\\
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,  \displaystyle
            \frac{\sum_j x_{ij} \,x_{ji}\, I\{v_j\,=\,v_i\}\, z_j }{\sum_j x_{ij}\,x_{ji}\, I\{v_j\,=\,v_i\} }$,\\
      where 0/0 is defined as 0;

\item \emph{total  reciprocated alter for same covariate  } effect on behavior $z$
 \texttt{(totRecAltSameX)},\\
      defined as the product of $i$'s behavior $z_i$ and
      the sum of behavior of those to whom $i$ has a  reciprocated tie
      and who also have the same covariate value:\\
       $s^{\rm beh}_{i\vit}(x, z) = z_i \,
            \sum_j x_{ij}\,x_{ji} \, I\{v_j\,=\,v_i\} \, z_j $; \\
...
(also see (\ref{inalt_av3})), \\
 ...
 \item {\em alter's distance-two incoming covariate total} effect on behavior $z$ 
 \texttt{(totXInAltDist2)}, defined by  $i$'s
 ...
 \item {\em average total incoming covariate alter effect at distance 2} 
 \texttt{(avTXInAltDist2)}, defined by  $i$'s
 ...
 \item {\em total average incoming covariate alter effect at distance 2} 
 \texttt{(totAXInAltDist2)}, defined by  $i$'s
...
These have to be specified using \texttt{covar1} for the covariate
and \texttt{covar2} for the network, e.g.,
\begin{verbatim}
...
(i.e., \texttt{depvar} and \texttt{covar1} are the same),
...
\begin{equation}
  \breve w_h = \left\{\begin{array}{ll} \displaystyle
         ...
         0                                &  \text{ if } x_{+i} = 0  .
  \end{array}   \right.            \label{inalt_avwa}
\end{equation}
      leading to the effect \\
...
\paragraph{User-defined interaction effects}
...
\[
a_{i\vit}(x, z) \, =\, \sum_{j=1}^n z_j \,x_{ij}\, x_{+j};
\]
...
\[
a_{i\vit}(x, z)\, =\, \sum_{j=1}^n z_j\, x_{ij}\, v_j;
\]
...
\[
a_{i\vit}(x, z)\, = \,x_{+i} \, \frac{\sum_{j=1}^n z_j\, x_{ij}}{\sum_{j=1}^n x_{ij}};
\]
...
\[
a_{i\vit}(x, z) \, = \, v_{i} \, \frac{\sum_{j=1}^n z_j\, x_{ij}}{\sum_{j=1}^n x_{ij}}.
\]

\item \emph{Total In-Exposure at Distance 2} (\verb|totInExposureDist2|):
    \[
        a_{i10}(x, z) =  \sum_{h \neq i} z_h \sum_{j} x_{ij} x_{hj}
    \]
    This statistic sums, for each actor $i$, the number of indirect exposures to
    alters with attribute $z_h$ via shared connections at distance 2
    (i.e., through common alters $j$).

    Multiple versions of averaging have been implemented:

\item \emph{Average Total In-Exposure at Distance 2} (\verb|avTinExposureDist2|):
\[
      a_{i11}(x, z) = \frac{\sum_{h \neq i} z_h \sum_{j} x_{ij} x_{hj}}{\sum_{j} x_{ij}}
\]
This version averages the total in-exposure at distance 2 by the number of direct
connections of actor $i$. If $\sum_{j}x_{ij}=0$, the statistic is defined as 0.

\item\emph{Total Average In-Exposure at Distance 2} (\verb|totAInExposureDist2|):
\[
      a_{i12}(x, z) = \sum_{h \neq i} z_h \sum_{j}\frac{ x_{ij} x_{hj}}{\sum_{h'}
            x_{h'j} - x_{ij}}
\]
Here, for each shared alter $j$, the exposure is averaged over the number of other
actors (excluding $i$) connected to $j$. If the denominator is 0, the term is
defined as 0.

% The version with both types of averaging is not yet implemented.

\item\emph{Any In-Exposure at Distance 2} (\verb|anyInExposureDist2|):
\[
      a_{i13}(x, z) = \sum_{j}
      \begin{cases}
      1  & \text{if } x_{ij} \sum_{h \neq i} z_h  x_{hj} > 0 \\
      0 & \text{otherwise}
      \end{cases}
\]
This effect counts, for each alter $j$ connected to $i$, whether there is at
least one other actor $h$ (with $z_h>0$) also connected to $j$. This is a
version of penalizing many indirect alters.

\item two \emph{GWDSP (geometrically weighted dyadwise shared partners) exposure 
      at Distance 2} effects,
{\em GWDSPFB exposure} (\verb|gwdspFBExposure|)  and
{\em GWDSPFF exposure} (\verb|gwdspFFExposure|) :
\[
      a_{i14}(x, z) = \sum_{h \neq i} z_h \,
            e^{\alpha}\Big\{1 - \big(1 - e^{-\alpha}\big)^{\sum_j x_{ij} x_{hj}}\Big\}
\]
the FF variant replaces $x_{hj}$ by $x_{jh}$. This is the geometrically weighted,
diffusion-rate counterpart of \verb|totInExposureDist2|: additional shared
partners to the same source $h$ are damped, so with strong weighting
($\alpha \to 0$) each distance-2 source $h$ with $z_h>0$ counts once.

\item two \emph{GWDIST2-weighted in-exposure} effects at distance 2,
{\em GWDIST2 exposure FF} (\verb|gwdist2FBExposure|)  and
{\em GWDIST2 exposure FB} (\verb|gwdist2FFExposure|):
\[
      a_{i15}(x, z) = \sum_j x_{ij} \,
            e^{\alpha}\Big\{1 - \big(1 - e^{-\alpha}\big)^{\sum_{h \neq i} x_{hj} z_h}\Big\}
\]
with $x_{jh}$ in place of $x_{hj}$ for the FF variant. The weighting here sits on
the behaviour accumulated behind each direct alter $j$ (exposure saturation
through a single gateway): with strong weighting ($\alpha \to 0$) it counts the
direct alters $j$ leading to at least one adopter, a flexible variant of
\verb|anyInExposureDist2|.
...
changed to $a_{i\vit}(x, z) = 0$ if less than $p$ alters
...
For the effects \verb|avExposure|, \verb|avTinExposureDist2|, \verb|totExposure|,
\verb|totInExposureDist2|, \verb|totAInExposureDist2|, \verb|susceptAvIn|,
and \verb|susceptAvCovar|, \\
if $p < 0$, the same rule is applied with the minimum $|p|$, but if there
...
It should be noted that by default all variables are internally centered by \rs.
The mean values used for the centering
are given near the beginning of the file produced by
\sfn{write\_report} and are given by the \textsf{"mean"}
...
From the output of \sfn{write\_report} for these data it can be seen that
...
It can be calculated by the \RS function \sfn{interpret\_selection}.

Given that this script is available in the working directory,
the data set is called \texttt{G129\_Data}, the answer
(\sfn{sienaFit}) object is called \texttt{ans5}, the network \texttt{"friendship"},
and the actor variable \texttt{"drinking"}, the following command
can be used.
...
done using package \sfn{autograph} \citep{autograph}.

This produces Figure~\ref{F_sel_dk} (colors to be improved).

\begin{figure}[htb]
  \centering
  \includegraphics[width = 0.5\textwidth]{selectionTableDrinking.png}
...
Interpreting the results along the lines of  \citet{SL2019},
...
2.99 + 0.024/(2 \times 0.113) = 3.1$, quite a middle value.
We see that the location of the maximum of the curves increases with
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.5\textwidth]{selectionTableDrugs.png}
...
in Section~\ref{S_f_b}, and it must not be forgotten that all covariates
are by default internally centered in \rs, and the subtracted means are reported
in the initial output produced by \textsf{write\_report},
...
The \emph{Influence Table} is defined as the matrix with in the rows
...
\]
The influence table can be produced by the function
\texttt{interpret\_influence}, which can be plotted
by package \sfn{autograph} \citep{autograph}.
...
somewhat less strong than in the case where all the friends drink a lot.

A figure of this table can be produced by package \sfn{autograph},
and is presented in Figure~\ref{F_infl_dk}.

\begin{figure}[hb]
  \centering
  \includegraphics[width=0.7\textwidth]{influenceTableDrinking.png}
...
Function \sfn{interpret\_influence} produces the following table,
which again can be plotted by \sfn{autograph}.
...
\begin{figure}[hb]
  \centering
  \includegraphics[width=0.8\textwidth]{influenceTable_sDrinking.png}
...
are available in \RS through function  \sfn{interpret\_size}.
...
They are calculated by  \sfn{interpret\_size} as \texttt{RHActors}
(see the help page). The mean over actors is printed by the print method
for \sfn{interpret\_size}.

Note that the values given by \sfn{interpret\_size} refer to each of the periods,
including the first wave; for this purpose, $x$ in (\ref{explained})
is taken as the observed network (and behaviour...) at the start of each period.
...
These standard deviations are computed by \sfn{interpret\_size} as \texttt{sigma}
and  printed by the print method for \sfn{interpret\_size} if \texttt{printSigma = TRUE}.
...
produced by \sfn{interpret\_size} if \texttt{getChangeStats = TRUE};
see the help page for \sfn{interpret\_size}.
...
can be calculated by \sfn{interpret\_size}.
...
The answer in this paper operates by replacing, in turn,
each parameter value $\beta_k$ by 0, and then considering the differences
...
for each actor always is 1), or raw. In the current implementation,
only the relative importances for the waves at the start of each period are
calculated as the first ministep after the last observation is never estimated
by \sfn{interpret\_size}.
The relative importances are calculated by  \sfn{interpret\_size} as \texttt{RIActors},
the raw importances as \texttt{IActors} (see the help page).
The  relative importances can be averaged over actors to give
\texttt{expectedRI}, which is printed by the print method for \sfn{interpret\_size}.

A weak point of this measure for importance of effects, or relative importance,
is the difficulty of grasping the meaning of the model defined by
probabilities where some parameter values are replaced by 0.
Setting one of the parameters to zero, while leaving the others unaffected,
leads to model specifications that are hard to interpret
and often are misleading.
For example, for a model including covariates, it is easy to see that
the relative importances depend on whether the covariates are centered.
More generally, simply setting a parameter to zero affects the variance
of the values of the objective function in any ministep decision.
...
If the function is called, e.g., \textsf{siena}, you can write
...
\noindent
This can happen in function \textsf{siena};
in conditional estimation (\texttt{cond = FALSE} in\\
 \textsf{set\_algorithm\_saom}),
...
\item If the model includes the \texttt{outRate} effect, consider
    ...
    (\texttt{cond = FALSE} in \sfn{set\_algorithm\_saom}).
\item If you are working with a data set with more than two waves, there might
   ...
   separately.
\item Set the parameter \texttt{firstg} in \textsf{set\_algorithm\_saom} to a
   ...
   parameter in \textsf{siena}. If the problem still occurs for \texttt{firstg} = 0.02,
...
\item In combination with trying out a lower value for \texttt{firstg},
    ...
    in \textsf{set\_algorithm\_saom}. Double averaging is a different definition
...
Calls: multi\_siena -> initializeBayes
...
\begin{indentation}{0.04\textwidth}{0pt}
\noindent
This can happen at the end of estimation in function \textsf{siena},
...
given as the output of \textsf{write\_report}).\\
...
multiple group data set (perhaps using \textsf{multi\_siena})
...
\texttt{allowOnly = FALSE} in the call of \textsf{as\_dependent\_rsiena},
...
\textsf{siena}; or the score test requested in function
\textsf{test\_time}.
In the first case it indicates that there are linear dependencies in the
list of effects (estimated and fixed)
that are used for \textsf{siena}.
If this error message occurs for \textsf{test\_time}, it indicates
linear dependencies in the list of effects estimated
in the \textsf{siena} run analyzed by this \textsf{test\_time},
...
\noindent
\emph{Solutions.} \\
For \textsf{siena}: drop some of the requested score-type test,
retaining only a set of tested effects between which there are no
linear dependencies.\\
For \textsf{test\_time}: exclude some of the requested
time heterogeneity tests by the \sfn{excludeEffects} parameter.
\end{indentation}

\subsection{In test\_gof}
...
\begin{indentation}{0.04\textwidth}{0pt}
\noindent
This can happen directly when calling \textsf{test\_gof}.
It indicates that you used a wrong name for \sfn{groupName} or \sfn{varName}.
See the help file for \textsf{test\_gof}.
\smallskip
...
\verb|R CMD check RSiena_1.6.9.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.6.9.tar.gz| \\
...
In function \sfn{set\_effect}, occurrences of \texttt{"\#"}
...
call of \sfn{set\_effect} is made with a new value of the
...
\begin{itemize}
  ...
  \item \texttt{"-{\char32}\#"}
\end{itemize}
where \texttt{"\char32"} denotes one blank space (used here for the sake
...
\item To choose a template for \sfn{TruncatedOutdegreeEffect},
      ...
      The argument \sfn{this} is a pointer referring to the object
...
some peculiarities.
Recall that effectGroups define combinations of dependent variable, 
effect type, and covariates (if any). 
They are used in the file \texttt{allEffects.csv}
and in function \sfn{effects.r}.
In the output of \sfn{effectsDocumentation()} 
(mentioning no effects object), the effectGroups are 
given as categories of the effects.

In the function \textsf{make\_specification} in file \sfn{effects.r}, 
some additional
measures are taken for effects in effectGroup \sfn{covarBipartiteObjective},
because this effect group does not differentiate between effects of covariates
defined on the first or on the second mode.
This implies that for adding such effects, it will be necessary to see whether
this function (i.e., \textsf{make\_specification}) also must be modified;
this will have to be done
in function \sfn{covarBipartiteEff} in file \sfn{effects.r}.
...
\item \sfn{doubleCovarNetObjective} is for effects where the
    ...
    coded in \sfn{effects.r}, \sfn{make\_data\_rsiena.r},
...
For effects defined for two dependent networks,
$X$ in the role of dependent network and $W$ in the role
of explanatory network, and one actor covariate $V$,
the following effectGroups are defined:
\begin{itemize}
\item \sfn{covarNetNetObjective} is for effects where
        both networks are one-mode;
\item \sfn{covarANetNetObjective} is for effects where
        $X$ is one-mode, and if $W$ is two-mode, 
        $V$ is defined for the first mode;
\item \sfn{covarAWNetNetObjective} is for effects where
        $W$ is one-mode, and $V$ is defined for the first mode;
\item \sfn{covarADNetNetObjective} is for effects where,
        if $W$ is two-mode, $V$ is defined for the first mode,
        and both networks should have the same second mode;
\item \sfn{covarBNetNetObjective} is for effects where,
        if $W$ is two-mode,
         $V$ is defined for the second mode;
\item \sfn{covarBXNetNetObjective} is for effects where,
        if $W$ is two-mode,
        $X$ should be one-mode, and $V$ is defined for the second mode.
\end{itemize}
\medskip

For effects defined for two dependent networks,
$X$ in the role of dependent network and $W$ in the role
of explanatory network, and a dyadic covariate,
the following effectGroups are defined:

\begin{itemize}
\item \sfn{dyadANetNetObjective} is for combinations
    of two networks and a dyadic covariate, where the network in the role
    of dependent network ($X$) should be one-mode, and the dyadic covariate
...
\item 2026-mm-dd GitHub, package version 1.6.x.
...
\begin{itemize}
  ...
  \item Corrections of documentation:
    \begin{itemize}
    \end{itemize}
  \item Corrections related to sanity checks:
    \begin{itemize}
    \end{itemize}
\end{itemize}
\fi
...
\item 2026-07-12 GitHub, package version 1.6.11.

Changes in RSiena:
\begin{itemize}
  \item Effects:
    \begin{itemize}
  \item New \texttt{activity alter} effect and non-centered variant
    of \texttt{popularity alter}.
  \item New alter-wise geometrically weighted distance-2 behavior effect
    \texttt{totGwInAltDist2\_nc}.
  \item New \texttt{totGwdspFF}/\texttt{totGwdspFB} (+ \texttt{\_nc}) effects,
    network-only structural controls for \texttt{totGwdspFFAlt}/\texttt{totGwdspFBAlt}.
  \item New covariate \texttt{gwdspFFX}/\texttt{gwdspFBX} (+ \texttt{\_nc})
    and \texttt{gwdist2XFF\_nc}/\texttt{gwdist2XFB\_nc} effects, building blocks
    for weighted indirect homophily.
  \item New diffusion-rate effects \texttt{gwdspFFExposure}/\texttt{gwdspFBExposure}
    and \texttt{gwdist2FFExposure}/\texttt{gwdist2FBExposure}.
  \item Group-level effects (\texttt{avGroup}/\texttt{totGroup} family) corrected
    and expanded, with a \texttt{resolvedValue} option for centered/non-centered
    behavior effect variants.
  \item Added non-centered (\texttt{\_nc}) variants of \texttt{altX}, \texttt{egoX},
    \texttt{egoXaltX}, \texttt{totInDist2}, \texttt{gwInAltDist2} as lower-order
    controls; covariate effects now more consistently respect non-centering.
    \end{itemize}
  \item Coding:
    \begin{itemize}
  \item \texttt{src/sources.list} restructured to a multi-line, more easily
    editable format, with an automated update script; \texttt{configure}
    updated accordingly, avoiding a build-time dependency on shell globbing.
  \item Several non-centered effect classes consolidated into an \texttt{nc}
    constructor flag on the corresponding main effect class.
    \end{itemize}
\end{itemize}


\item 2026-06-10 GitHub, package version 1.6.9.

Changes in RSiena:
\begin{itemize}
  \item Effects:
    \begin{itemize}
  \item Selection of effects for combinations of two dependent networks
    and an actor covariate improved, which leads to including
    more effects for such combinations where one of the dependent networks
    is bipartite (see `coding' below).
    \end{itemize}
  \item Improvements of functionality:
    \begin{itemize}
  \item New function \sfn{estimate\_onestep}.
  \item Option \texttt{onestep} for \sfn{update\_theta.sienaEffects} changed.
  \item Hidden option to use the one-step estimator with 
      \texttt{prevAns} in \sfn{siena} (experimental).
  \item \sfn{print.sienadata} and \sfn{print.sienaGroup} now specify 
    the node sets for covariates and restricts printed line lengths to 64.
  \item More informative error message in \sfn{make\_data\_rsiena} if the number
    of elements of the second node set for bipartite networks is not correct.
  \item In \sfn{transformScript}, the replacement for \sfn{descriptives.sienaGOF}
    was put before the replacement for \sfn{sienaGOF}.
    \end{itemize}
  \item Coding:
    \begin{itemize}
  \item Reorganization and clarification of \texttt{effectGroups} for combinations
    of two dependent networks and an actor covariate:
    these now are \texttt{covarNetNetObjective}, \\
    \texttt{covarANetNetObjective}, \texttt{covarAWNetNetObjective},\\
    \texttt{covarADNetNetObjective}, 
    \texttt{covarBNetNetObjective},\\
    \texttt{covarBXNetNetObjective}.
    See Section~\ref{S_notestwomode}.
    \end{itemize}
\end{itemize}


\item 2026-05-15 GitHub, package version 1.6.7.

Changes in RSiena:
\begin{itemize}
\item Improvements of functionality:
   \begin{itemize}
     \item Option \texttt{onestep}  for \sfn{test\_parameter(..., method="score", ...)} .
     \item Option \texttt{onestep}  for \sfn{update\_theta}.
  \item Name \texttt{sienaNet}  added to \sfn{transformScript}  as one of the names
    to be replaced.
  \item \sfn{make\_effects}  now also works for \texttt{sienadata}
  objects older than 1.4.10.
   \end{itemize}
\end{itemize}

\item 2026-04-20 GitHub and CRAN, package version 1.6.6.

Changes in RSiena:
\begin{itemize}
\item Corrections related to sanity checks:
   \begin{itemize}
     \item Stack handling improved in
       \texttt{siena07utilities.cpp} for compliance with CRAN.
   \end{itemize}
\end{itemize}


\item 2026-04-08 GitHub, package version 1.6.5.

Changes in RSiena:
\begin{itemize}
\item Corrections related to sanity checks:
   \begin{itemize}
     \item  Keyword handling corrected in help files for
       \sfn{interpret\_influence}, \sfn{interpret\_selection},
       \sfn{make\_specification}, \sfn{set\_effect},
       \sfn{test\_gof\_auxiliary} and \sfn{test\_time}.
     \item In help file for \sfn{interpret\_size\_dynamics}, part of
       example code disabled for testing due to time exceeding 5
       seconds in checks.
   \end{itemize}
\end{itemize}


\item 2026-04-07 GitHub, package version 1.6.4.

Changes in RSiena:
\begin{itemize}
\item Bug corrections:
  \begin{itemize}
    \item For \sfn{test\_gof}, a bug in the handling
      of auxiliary functions was fixed.
  \end{itemize}
\end{itemize}

\item 2026-03-21 GitHub, package version 1.6.3.

Changes in RSiena:
\begin{itemize}
\item New effects:
   \begin{itemize}
  \item New effects \texttt{outdegMixedPop} and \texttt{indegMixedPop}.
  \item \texttt{endowment} allowed for \texttt{XWX1}, \texttt{XWX2}, and \texttt{cl.XWX1}.
    \end{itemize}
\item Improvements of functionality:
   \begin{itemize}
  \item Auxiliary function \sfn{egoAlterCovarComb} for \sfn{test\_gof}.
  \item Option \texttt{splitDepvars} for \sfn{set\_algorithm\_saom}
    for making zero blocks in $D$ for different-dependent variable
    effects.
  \item The quasi-Newton Raphson update step at the end of Phase 1 used
    now uses the partially diagonalized (and possibly zero blocks) version of $D$,
    unless \texttt{splitDepvars}=-1.
  \item Column names \texttt{inter1} and \texttt{inter2} of the result of \sfn{effectsDocumentation}
    changed to \texttt{covar1} and \texttt{covar2}.
  \item Better error message for \sfn{set\_interaction} in case \texttt{shortNames}
    is not a list.
  \item In \sfn{sienaTest} objects, if the test is one-dimensional, the standard error
    of the linear combination is given as an additional component \texttt{sterror}
    and included in \sfn{print.sienaTest}.
    \end{itemize}
\item Bug corrections:
   \begin{itemize}
  \item For \sfn{transformScript}, processing of \sfn{Multipar.RSiena} added.
  \item For \sfn{test\_gof},
    all arguments are transferred to the call of \sfn{sienaGOF}.
  \item For \sfn{interpret\_selection},
    all arguments are transferred to the call of \sfn{selectionTable}.
  \item For \sfn{interpret\_influence},
    all arguments are transferred to the call of \sfn{influenceTable}.
    \end{itemize}
\item Documentation:
   \begin{itemize}
  \item Manual updated to new function names and use of \sfn{autograph} for
    plotting influence and selection tables.
  \item In help file for \sfn{set\_interaction}, treatment of \texttt{shortNames}
    corrected.
  \item In help file for \sfn{set\_effect}, treatment of \texttt{covar1} and \texttt{covar2}
    corrected.
    \end{itemize}
\end{itemize}

\item 2026-25-02 GitHub, package version 1.6.2.

Changes in RSiena:
\begin{itemize}
\item New function \sfn{transformScript}.
\item New method \sfn{write\_report} for \sfn{sienadata} objects,
    replacing \sfn{print01Report} (which still is retained).
\end{itemize}

\item 2026-01-20 GitHub, package version 1.6.1

Changes in RSiena:
\begin{itemize}
\item New effects:
   \begin{itemize}
   \item Effects \texttt{sameXOutAct}, \texttt{diffXOutAct}, \texttt{crossXOutAct} also implemented for two-mode networks.
    \end{itemize}
\end{itemize}

\item 2026-01-02 GitHub, package version 1.6.0

Changes in RSiena:
\begin{itemize}
\item New function names:
   \begin{itemize}
   \item Overhaul of naming system of exported functions.
        Old function names still are available.
        See Chapter~\ref{S_names}.
   \item Chapter 3 was inserted in the manual to explain
    the new function names.
   \item Class name \texttt{siena} for siena data sets
     replaced by \texttt{sienadata}.
    \end{itemize}
\item Coding:
   \begin{itemize}
    \item Completely refactored diffusion rate effects.
    \item For \sfn {sienaRI}: corrected contributions for the
    first potential ministep in each period, and dropped calculation of the
    first ministep after the last observation wave, and the
    RI measure at the last observation moment. \\
    Further see the \texttt{NEWS.md} file.
    \end{itemize}
\item New effects:
   \begin{itemize}
  \item New distance 2 exposure diffusion rate effects \texttt{totInExposureDist2},
  \texttt{totAInExposureDist2}, \texttt{anyInExposureDist2},
  behavior effects \texttt{totPopAlt},
  \texttt{totGroup}, \texttt{indegAvGroup}, \texttt{indegtotGroup},
  \texttt{totGwdspFFAlt}, \texttt{totGwdspFBAlt\_nc} (Daniel Gotthardt).
  \item New behavior effects without centering \texttt{quad\_nc},
  \texttt{avInAltDist2\_nc}, \texttt{totInAltDist2\_nc}, \\
  \texttt{avTinaltDist2\_nc}, \texttt{totAInAltDist2\_nc},
  \texttt{totGwdspFFAlt\_nc}, \texttt{totGwdspFBAlt\_nc}.
 \item New effects \texttt{avSameXAlt}, \texttt{totSameXAlt},
 \texttt{avSameXInAlt}, \texttt{totSameXInalt},
  \texttt{avXRecAlt}, \texttt{totXRecAlt}, \texttt{avSameXRecAlt},
  \texttt{totSameXRecAlt},
  \texttt{avAltSameX}, \texttt{totAltSameX}, \texttt{avRecAltSameX},
  \texttt{totRecAltSameX}.
  \item New effects \texttt{avInAltAltX}, \texttt{totInAltAltX},
  \texttt{avRecAltAltX}, \texttt{totRecAltAltX},
    \texttt{avInAltSameX}, \texttt{totInAltSameX}.
    \end{itemize}
\item Improvements of functionality:
   \begin{itemize}
    \item Distinguished between three algorithm objects.
    See Chapter~\ref{S_names}.
    \end{itemize}
\item Bug corrections:
   \begin{itemize}
  \item \texttt{checkImpossibleChanges}  restricted to networks.
  \item \texttt{checkZeroChanges}  adapted to also cover behavioral variables.
    \end{itemize}
\item Documentation:
   \begin{itemize}
  \item A new subsection was added to \texttt{Siena\_algorithms}  to explain the new
    construction of diffusion rate effects (Daniel Gotthardt).
   \item Appendix A of this manual, "List of Functions in Order of Execution",
    was dropped
...
\item 2025-10-28 GitHub, package version 1.5.6.

Changes in RSiena:
\begin{itemize}
\item Improvements of functionality:
   \begin{itemize}
  \item Auxiliary function \sfn{egoAlterCombi} for  Goodness of Fit
    now omits values  where the behavioral variable is missing.
    \end{itemize}
\item Bug corrections:
   \begin{itemize}
   \item Bug corrected that may have affected effects
   \texttt{sameXInPop}, \texttt{diffXInPop},
   \texttt{sameXOutAct}, \texttt{diffXOutAct}, and \texttt{sameXVInPop},
   leading to divergence or crashes.
    \end{itemize}
\end{itemize}


\item 2025-10-17 GitHub, package version 1.5.3.

Changes in RSiena:
\begin{itemize}
\item New effects:
   \begin{itemize}
    \item \texttt{quad\_cc}, \texttt{avAlt\_cc}, \texttt{totAlt\_cc}.
    \end{itemize}
\end{itemize}


\item 2025-09-06 GitHub, package version 1.5.2.

Changes in RSiena:
\begin{itemize}
\item Improvements of functionality:
   \begin{itemize}
    \item Option \texttt{returnChangeContributions=TRUE} added to
        \sfn{siena07}, allowing users to extract the
        \texttt{changeContributions} to the \sfn{sienaFit} object.
    \item \sfn{interpret\_size\_dynamics} reinstated based on the above new option,
        replacing the original code.
  \item \sfn{selectionTable} got an attribute \texttt{quad} indicating
    whether the plot is a quadratic function.
  \item Users can now extract now extract the changeContributions when running
    \sfn{siena07} by setting an argument \texttt{returnChangeContributions=TRUE}.
    If used together with \texttt{nsub=0} and \texttt{prevAns} or modified initial
    values in the effects object, especially useful for post-estimation,
    e.g. in \sfn{interpret\_size\_dynamics}.
    \end{itemize}
\item New effects:
   \begin{itemize}
    \item \texttt{outActMore\_ego}, \texttt{outActSqrtMore\_ego}, \texttt{outMore\_ego},
        \texttt{outPopMore}, \\
        \texttt{outPopSqrtMore}, \texttt{outPopThreshold}.
    \end{itemize}
\end{itemize}
...
\item Improvements of functionality:
  ...
  \item Option \texttt{prML=2} for maximum likelihood estimation
    using the \texttt{move} proposal step reinstated (\sfn{set\_algorithm\_saom}).
    \end{itemize}
\item Effects:
   \begin{itemize}
