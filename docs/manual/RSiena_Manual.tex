...
This version was renewed for \RS version 1.3.28.
\end{abstract}
...
Manual for \textsf{RSiena} (version \today).
Oxford: University of Oxford, Department of Statistics; Nuffield College.
{\newline{\url{http://www.stats.ox.ac.uk/~snijders/siena/RSiena_manual.pdf}}},
\smallskip
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.3.28).
...
\item \emph{in-isolate Outdegree effect}, (\texttt{inIsDegree}), the
 ...
 \texttt{outMore} and \texttt{outMore2}, to allow the simultaneous use of this
 effect with 2 different internal effect parameters;
 they are defined by \\
...
\verb|R CMD check RSiena_1.3.28.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.3.28.tar.gz| \\
...
The \texttt{NEWS.md} file can also be consulted during an \R session in which you
have loaded the package \rs, provided you have also attached the
...
\item 2023-mm-dd GitHub, package version 1.x.x.
...
\item Changed effect:
   \begin{itemize}
  \item \texttt{threshold}, \texttt{threshold2}, \texttt{threshold3} and \texttt{threshold4} changed to work with non-centered parameter (not backward-compatible).
    \end{itemize}
\end{itemize}
