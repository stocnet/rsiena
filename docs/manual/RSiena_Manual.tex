...
Snijders05}, \citet*{SnijdersEA07}, \citet*{SnijdersEA10a},
and \citet{KoskinenSnijders2022}.
...
This version was renewed for \RS version 1.3.9.
\end{abstract}
...
Melbourne, Geneva, and elsewhere.

In addition to the `official' \R distribution of \RS
and the \sfn{GitHub} development version, there is
an additional distribution \sfn{RSienaTest} at \sfn{R-Forge},
which includes additional options that are still in the testing stage.
...
\url{http://cran.r-project.org/web/packages/RSiena/} .\\
            ...
            or \sfn{RSienaTest} from GitHub or the \SI website.
            You can go to\\
...
and choose \texttt{Install From: Package archive file (zip; tar.gz)}.
            ...
            Installation of \texttt{RSienaTest}
            from the R-Forge repository can be done as follows.
            \begin{itemize}
            \item for Windows:\\
            ...
            If this does not work, download the binary for your Operating System and
            install it, as indicated above for \sf{RSiena}.
            \end{itemize}
   \end{enumerate}
...
\item[Check your version of \RS]
  ...
  The version is
  identified by a version number (e.g.\ 1.3.9).
...
\item \emph{in-alter's  total of dyadic covariate W } effect on behavior $z$
 \texttt{(totWInAlt)},\\
      ...
      the sum of $i$'s in-alters' values of the dyadic covariate, \\
       $s^{\rm beh}_{i\vit}(x, z) = z_i \, \sum_h x_{hi}w_{hi}  $.\\
 ...
 (and the mean behavior, i.e. $0$, if the ratio is 0/0) ;
  ...
  \item {\em average in-alter weighted by dyadic covariate $W$}
                       ...
                       depending on the parameter value (1 or 2):\\
  parameter = 1 : \ $s^{\rm beh}_{i\vit}(x, z) =
                     z_i \big( \sum_j x_{ji} \, w_{ji} \, z_j \big)  /
                                        \big (\sum_j x_{ji}\big)$\\
   parameter = 2 : \  $s^{\rm beh}_{i\vit}(x, z) =
                    z_i \big( \sum_j x_{ji} \, w_{ji} \, z_j \big)  /
                                \big (\sum_j w_{ji} \,x_{ji}\big) ;$\\
 (and the mean behavior, i.e. $0$, if the ratio is 0/0) ;
 ...
 \item {\em total in-alter weighted by dyadic covariate $W$}
                       \texttt{(totInAltW)},
                       a weighted version of  \texttt{totAlt};\\
   $s^{\rm beh}_{i\vit}(x, z) =  z_i \, \sum_j x_{ji} \, w_{ji} \, z_j  ;$
...
\verb|R CMD check RSiena_1.3.9.tar.gz|\\
...
to others, type\\
\verb|R CMD INSTALL --build RSiena_1.3.9.tar.gz| \\
...
\item 2022-mm-dd GitHub, package version 1.x.x.
...
\item 2022-03-23? GitHub, package version 1.3.9.1??

Changes in RSiena:
\begin{itemize}
\item  Effects:
\begin{itemize}
  \item New effects  \texttt{avInAltW}, \texttt{avWInAlt}, \texttt{totInAltW}, \texttt{totWInAlt}.
\end{itemize}
