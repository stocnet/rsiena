...
\item In this phase a small number $n_1$ of steps are made to estimate\\
     ...
     At the end of Phase 1, calculate the following results:
      ...
      \item If \nnm{splitDepvars}=1 in the algorithm, then replace the
        blocks in matrix $ \tilde{D}$ regarding effects for different dependent
        variables (which occur only for co-evolution models) by zero blocks.
      ...
      \[
       {\hat{\theta}} = \theta_1 -
         a_1 {\tilde{D}}^{-1} \left( \bar{s} - s \right)\, .
      \]
      where
      \[
      \nnm{targets} = s = \text{ observed values} .
      \]
      (For this step, before version 1.6.3, the matrix $\hat{D}$ was used
      instead of $\tilde{D}$.)
      \end{enumerate}
      \medskip

     In the code, matrix $\hat{D}$ is \nnm{dinv} and $\tilde{D}$ is \nnm{dinvv}.
\item[Phase 2.]
...
\subsection{Endowment and creation effects for behavior}

Function \nnm{StatisticCalculator::calculateBehaviorStatistics} contains the line
\begin{verbatim}
 difference[i] =
	pBehaviorData->value(this->lperiod, i) - currentState[i];
\end{verbatim}
This means that \texttt{difference[i]} is +1 if the simulated value is lower,
and --1 if the simulated value is higher than the observed value at the
start of the period.

Therefore, in all functions \texttt{egoEndowmentStatistic},\\
the condition \texttt{ (difference[ego] > 0)} is required.

In function \nnm{BehaviorEffect::creationStatistic}, \\
the negative value
of \texttt{egoEndowmentStatistic} is returned.
...
I hope this is correct. I have multiplied together the terms for each ego
...
in \nnm{StatisticCalculator.cpp}
calculate the rate functions. In addition to calculating rate statistics,
RSiena also computes scores for rate effects. These scores are calculated in
functions such as \nnm{getStatistics} in \texttt{siena07internals.cpp} and in
...
\subsection{Diffusion rate effects\protect\footnote{This
    section was contributed by Daniel Gotthardt.}}

Diffusion rate effects can be added and modified more easily than other rate
effects, as they are implemented as a distinct class, \nnm{DiffusionRateEffect},
which inherits from \nnm{BehaviorRateEffect}. The method
\nnm{DiffusionRateEffect::calculateRateContribution} returns the value used in
the calculation of rates, scores, and statistics in both \nnm{DependentVariable}
and \nnm{StatisticCalculator}, each of which creates its own instances of
diffusion rate effects.

The contribution to the rate function is calculated by\\
\nnm{DiffusionRateEffect::proximityValue}, which must be defined in each
diffusion rate class (or is otherwise pure virtual). To create a new
\nnm{DiffusionRateEffect}, one should define a new class inheriting from
\nnm{DiffusionRateEffect} and implement the mathematical definition of the
rate effect in the \nnm{proximityValue} method. New diffusion rate effects
must also be explicitly constructed and initialized in
\nnm{DependentVariable::initializeRateFunction} and
\nnm{StatisticCalculator::calculateBehaviorRateStatistics}. Additionally,
they must be registered as known diffusion rate effects in the \nnm{getStatistics}
function in \texttt{siena07internals.cpp}.

Further refactoring of all rate effects and their construction and storage may
be possible to make the code more uniform and easier to maintain.
